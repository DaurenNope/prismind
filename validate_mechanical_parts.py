...
def print_test(name: str, status: str, error: Optional[str] = None, details: Optional[Dict] = None):
    if status == "PASS":
...
def test_environment() -> TestResult:
    ...
    try:
        ...
        if missing_required:
            ...
            result.passed = True
            if missing_optional:
                result.warnings.append(f"Missing optional vars: {', '.join(missing_optional)}")
...
async def test_collectors_initialization() -> TestResult:
    ...
    if all(collectors.values()):
        ...
        result.passed = True
        result.warnings = [f"{k}: {v}" for k, v in errors.items() if not collectors.get(k)]
    else:
...
async def test_analyzer_initialization() -> TestResult:
    ...
    try:
        ...
        services = {
            "ollama": analyzer._ollama_client is not None if hasattr(analyzer, '_ollama_client') else False,
            "mistral": analyzer._mistral_client is not None if hasattr(analyzer, '_mistral_client') else False,
            "gemini": analyzer.gemini_model is not None if hasattr(analyzer, 'gemini_model') else False,
        }
        
        # Check ai_services list
        if hasattr(analyzer, 'ai_services') and analyzer.ai_services:
            available_services = [s.get('name') for s in analyzer.ai_services if s.get('name')]
        else:
        ...
        if not any(services.values()) and not available_services:
            ...
            if len(available_services) < 3:
                result.warnings.append(f"Only {len(available_services)}/3 AI services available: {available_services}")
