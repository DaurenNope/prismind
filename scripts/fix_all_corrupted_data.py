...
def fix_sqlite_database(limit=None):
    ...
    db_path = project_root / 'prismind.db'
    ...
    for row in rows:
        ...
        for field in ['tags', 'key_concepts', 'rewrite_reasons', 'rewrite_risks', 'best_persona_reasons', 
                      'persona_fit_scores', 'persona_fit_reasons', 'hashtags', 'mentions', 'time_sensitive_reasons']:
        ...
        has_analysis = post_data.get('ai_summary') and str(post_data.get('ai_summary', '')).strip()
            ...
            try:
                ...
                if result.rowcount > 0:
                    ...
                    if updated_count % 10 == 0:
                        logger.info(f"   Progress: {updated_count} posts updated in SQLite")
            except Exception as e:
                logger.error(f"Failed to update post {post_data.get('post_id')} in SQLite: {e}")
                import traceback
...
def fix_supabase_database(limit=None):
    ...
    try:
        ...
        if not db_agent._supabase:
            logger.warning("Supabase not available (check environment variables), skipping Supabase fix")
            return 0
                ...
                if db_agent._dupes:
                    class _DuplicateCheckerWrapper:
                        ...
                        def check_duplicate_post(self, content, author, platform, url=None):
                            item = {"content": content, "author": author, "platform": platform, "url": url}
                            return self.detector.is_duplicate(item)
                    duplicate_checker = _DuplicateCheckerWrapper(db_agent._dupes)
                db_agent._post_inserter = PostInserter(db_agent._supabase, duplicate_checker)
        ...
        for idx, row in enumerate(rows, 1):
            ...
            try:
                ...
                if idx % 10 == 0:
                    logger.info(f"   Progress: {idx}/{len(rows)} posts processed ({updated_count} successful, {error_count} errors)")
            except Exception as e:
                error_count += 1
                logger.error(f"Failed to update Supabase post {post_data.get('post_id')}: {e}")
                # Continue processing other posts even if one fails
        
        logger.info(f"✅ Fixed {updated_count} posts in Supabase ({error_count} errors, {len(rows) - updated_count - error_count} skipped)")
        return updated_count
...
def main():
    ...
    parser.add_argument('--supabase-only', action='store_true', help='Only update Supabase')
