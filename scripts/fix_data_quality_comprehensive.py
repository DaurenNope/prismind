...
class DataQualityFixer:
    ...
    def validate_post(self, post_data: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        if post_data.get('ai_summary'):
            ...
            valid_categories = ['TECH', 'DATING', 'CRYPTO', 'BUSINESS', 'LEARNING', 
                              'NEWS', 'PERSONAL', 'HEALTH', 'ENTERTAINMENT', 'OTHER', 'DEPRECATED']
            ...
            if persona_key:
                ...
                invalid_persona_values = ['evergreen', 'timely', 'trending', 'breaking', 
            ...
            # 6. Check JSON fields
            json_fields = ['tags', 'key_concepts', 'rewrite_reasons', 'rewrite_risks',
                          'persona_fit_scores', 'persona_fit_reasons', 'best_persona_reasons']
            ...
            for field in bool_fields:
                ...
                if value is not None and not isinstance(value, bool):
                    ...
                    if isinstance(value, str):
                        if value.lower() not in ['true', 'false', '1', '0', 'yes', 'no', 't', 'f', 'neutral']:
    ...
    def _is_valid_timestamp(self, value: Any) -> bool:
        ...
        if isinstance(value, str):
            ...
            invalid_values = ['mistral', 'gemini', 'ollama', 'qwen', 'ai_service', 
                            'unknown', 'none', 'null', '']
    ...
    def fix_post(self, post_data: Dict[str, Any]) -> Tuple[Dict[str, Any], List[str], List[str]]:
        """
    ...
    def fix_all_posts(self, dry_run: bool = False, limit: int = None, verbose: bool = False) -> Dict[str, Any]:
        """
        ...
        for idx, row in enumerate(rows, 1):
            ...
            for field in ['tags', 'key_concepts', 'rewrite_reasons', 'rewrite_risks',
                         'best_persona_reasons', 'persona_fit_scores', 'persona_fit_reasons',
                         'hashtags', 'mentions', 'time_sensitive_reasons', 'embedding']:
            ...
            try:
                ...
                for issue in issues_before:
                    self.stats['issues_found'][issue] = self.stats['issues_found'].get(issue, 0) + 1
                
                for issue in issues_after:
                    self.stats['issues_fixed'][issue] = self.stats['issues_fixed'].get(issue, 0) + 1
                ...
                result = {
                    ...
                    'before': {k: v for k, v in post_data.items() if k in ['category', 'best_persona_key', 'rewrite_score', 'value_score', 'quality_score', 'relevance_window', 'analyzed_at']},
                    'after': {k: v for k, v in fixed_data.items() if k in ['category', 'best_persona_key', 'rewrite_score', 'value_score', 'quality_score', 'relevance_window', 'analyzed_at']}
                }
                ...
                if has_changes:
                    ...
                    if verbose:
                        ...
                        logger.info(f"    Issues: {len(issues_before)} -> {len(issues_after)}")
                        if issues_before:
                            logger.info(f"    Before: {', '.join(issues_before)}")
                        if issues_after:
                            logger.info(f"    After: {', '.join(issues_after)}")
                        logger.info(f"    Changes: category={post_data.get('category')} -> {fixed_data.get('category')}, "
    ...
    def _update_post(self, post_data: Dict[str, Any]):
        ...
        try:
            ...
            update_fields = [
                'created_at', 'analyzed_at', 'category', 'best_persona_key',
                'best_persona_score', 'rewrite_score', 'rewrite_readiness',
                'rewrite_reasons', 'rewrite_risks', 'analysis_confidence',
                'analysis_depth', 'needs_deep_analysis', 'persona_fit_scores',
                'persona_fit_reasons', 'best_persona_reasons', 'value_score',
                'quality_score', 'urgency_score', 'time_sensitive',
                'relevance_window', 'time_sensitive_reasons', 'tags',
                'key_concepts', 'topic', 'content_type', 'language',
                'ai_summary', 'is_saved'
            ]
            ...
            for field in update_fields:
                ...
                if field in ['tags', 'key_concepts', 'rewrite_reasons', 'rewrite_risks',
                           'best_persona_reasons', 'time_sensitive_reasons']:
    ...
    def print_stats(self):
        ...
        if self.stats['issues_found']:
            print("\nIssues Found:")
            for issue, count in sorted(self.stats['issues_found'].items(), key=lambda x: -x[1]):
                print(f"  {issue:30s}: {count:4d}")
        
        if self.stats['issues_fixed']:
            print("\nIssues Fixed:")
            for issue, count in sorted(self.stats['issues_fixed'].items(), key=lambda x: -x[1]):
                print(f"  {issue:30s}: {count:4d}")
    ...
    def print_detailed_results(self):
        ...
        for result in self.stats['detailed_results']:
            ...
            print(f"  Issues before: {len(result['issues_before'])} - {', '.join(result['issues_before']) if result['issues_before'] else 'None'}")
            print(f"  Issues after: {len(result['issues_after'])} - {', '.join(result['issues_after']) if result['issues_after'] else 'None'}")
            print(f"  Has changes: {result['has_changes']}")
            
            if result['has_changes']:
                print("  Changes:")
                for key in ['category', 'best_persona_key', 'rewrite_score', 'value_score', 'quality_score', 'relevance_window']:
...
def main():
    ...
    parser.add_argument('--db', type=str, default='prismind.db', help='Database path')
    parser.add_argument('--verbose', '-v', action='store_true', help='Verbose output')
    parser.add_argument('--evaluate', '-e', action='store_true', help='Show detailed evaluation')
    ...
    stats = fixer.fix_all_posts(dry_run=args.dry_run, limit=args.limit, verbose=args.verbose)
    fixer.print_stats()
