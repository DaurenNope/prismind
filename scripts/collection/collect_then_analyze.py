#!/usr/bin/env python3
"""
Collect then Analyze approach for BEYONDLINES
...
async def analyze_only(limit: int = None) -> int:
    ...
    for i, post in enumerate(posts_to_analyze, 1):
        ...
        title = (
            post.get("title", "")[:50] + "..."
            if post.get("title") and len(post.get("title")) > 50
            else post.get("title", "")
        )
        print(
...
async def collect_then_analyze(platforms: List[str] = None, analysis_limit: int = None):
    ...
    return {"collection": collection_results, "analysis": analysis_results}
