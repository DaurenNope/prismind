...
async def reanalyze_posts(limit=None, force=False):
    ...
    for idx, row in enumerate(rows, 1):
        ...
        try:
            # Re-analyze the post
            logger.info(f"Re-analyzing post {post_data.get('post_id')} ({idx}/{len(rows)})...")
            result = await analyze_and_store_post(db_agent, post_data)
            
            if result:
                reanalyzed_count += 1
                if reanalyzed_count % 10 == 0:
                    logger.info(f"   Progress: {reanalyzed_count}/{len(rows)} posts re-analyzed")
            else:
                error_count += 1
...
async def main():
    """Main function"""
    import argparse
    parser = argparse.ArgumentParser(description='Re-analyze posts with missing or incorrect analysis data')
    parser.add_argument('--limit', type=int, help='Limit number of posts to re-analyze')
    parser.add_argument('--force', action='store_true', help='Force re-analysis of all posts')
