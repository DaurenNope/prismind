...
def check_all_edge_cases(post: Dict[str, Any]) -> List[Tuple[str, str]]:
    ...
    if post.get('relevance_window') == 'evergreen':
        ...
        if check_time_sensitive_keywords(content, ai_summary):
            issues.append(('time_sensitive_keywords', 'Contains time-sensitive keywords in evergreen content'))
        ...
        try:
            ...
            if created_dt:
                ...
                if age_days == 7 and post.get('relevance_window') == 'evergreen':
                    ...
                    issues.append(('age_validation', f'Evergreen post is too recent ({age_days} days old, need >=7 days)'))
                
                # Edge case: Exactly 6 months old (boundary condition)
                if age_months >= 6.0 and post.get('relevance_window') == 'evergreen':
                    issues.append(('age_validation', f'Evergreen post is too old ({age_months:.1f} months, need <6 months)'))
                
                # Edge case: Time-sensitive post >7 days old
                if post.get('relevance_window') in ('same-day', '24-72h', 'this-week'):
                    if age_days > 7:
                        issues.append(('age_validation', f'Time-sensitive post is too old ({age_days} days, need <=7 days)'))
    ...
    if rewrite_score <= 0:
        issues.append(('score_validation', f'rewrite_score is {rewrite_score} (must be > 0)'))
    if value_score <= 0:
        issues.append(('score_validation', f'value_score is {value_score} (must be > 0)'))
    if quality_score <= 0:
        issues.append(('score_validation', f'quality_score is {quality_score} (must be > 0)'))
    ...
    if post.get('relevance_window') == 'evergreen':
        ...
        if urgency_score >= 0.35:
            issues.append(('urgency_validation', f'Evergreen post has high urgency ({urgency_score:.2f} >= 0.35)'))
    
    # 9. Time-sensitive flag validation (for evergreen)
    if post.get('relevance_window') == 'evergreen':
        if post.get('time_sensitive', False):
            issues.append(('time_sensitive_flag', 'Evergreen post is marked as time_sensitive'))
    
    # 10. Required fields validation
    required_fields = ['post_id', 'platform', 'content', 'ai_summary', 'rewrite_score', 'value_score', 'quality_score', 'category', 'relevance_window']
    ...
    content = post.get('content', '') or ''
    if len(content) < 50:
        issues.append(('content_length', f'Content is too short ({len(content)} chars, need >=50)'))
    
    # 12. Check for suspicious patterns in content
    content_lower = content.lower()
    suspicious_patterns = [
        (r'\b(test|testing|example|sample|placeholder)\b', 'Contains test/placeholder language'),
    ...
    if 'placeholder' in ai_summary_lower or 'error' in ai_summary_lower[:100]:
        issues.append(('ai_summary_quality', 'AI summary appears to be placeholder or error'))
    
    return issues


def comprehensive_validation(db_path: str = 'prismind.db') -> Dict[str, Any]:
    ...
    for post in usable_posts:
        ...
        cur.execute('SELECT * FROM posts WHERE post_id = ? AND platform = ?', (post_id, platform))
        db_post = cur.fetchone()
        if not db_post:
            results['posts_with_issues'] += 1
            results['total_issues'] += 1
            results['issues_by_type']['not_found'] = results['issues_by_type'].get('not_found', 0) + 1
...
def main():
    ...
    if results['posts_with_issues'] == 0:
        ...
        print(f"\n❌ FOUND ISSUES:")
        print(f"   {results['posts_with_issues']} posts have issues ({results['posts_with_issues']/results['total_usable_posts']*100:.1f}%)")
