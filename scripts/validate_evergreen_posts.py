...
def analyze_evergreen_accuracy():
    ...
    conn = sqlite3.connect('prismind.db')
    ...
    if len(evergreen_posts) > 0:
        print(f"   False positive rate: {len(false_evergreen)/len(evergreen_posts)*100:.1f}%")
    
    # Show breakdown of false evergreen reasons
    if false_evergreen:
        print(f"\n❌ FALSE EVERGREEN BREAKDOWN:")
        urgency_count = sum(1 for f in false_evergreen if any('High urgency' in str(r) for r in f['reasons']))
        time_sensitive_count = sum(1 for f in false_evergreen if any('time_sensitive flag' in str(r) for r in f['reasons']))
        news_count = sum(1 for f in false_evergreen if any('NEWS category' in str(r) for r in f['reasons']))
        old_count = sum(1 for f in false_evergreen if any('Old' in str(r) or 'months old' in str(r) for r in f['reasons']))
    ...
    time_sensitive_posts = [p for p in posts if p['relevance_window'] and p['relevance_window'] != 'evergreen']
    ...
    print(f"   ✅ Truly evergreen (<6 months, not DEPRECATED): {sum(1 for u in usable_posts if u['reason'] == 'truly_evergreen')}")
    print(f"   ✅ Fresh time-sensitive (≤7 days, not DEPRECATED): {sum(1 for u in usable_posts if u['reason'] == 'fresh_time_sensitive')}")
    print(f"   ❌ DEPRECATED posts excluded: {deprecated_excluded}")
    print(f"   📊 TOTAL USABLE POSTS: {len(usable_posts)} ({len(usable_posts)/total*100:.1f}% of analyzed posts)")
    
    # Show age distribution of usable posts
    print(f"\n📅 USABLE POSTS AGE DISTRIBUTION:")
    evergreen_ages = [u['age_months'] for u in usable_posts if u['reason'] == 'truly_evergreen' and u['age_months'] is not None]
    fresh_ages = [u.get('age_days', 0) for u in usable_posts if u['reason'] == 'fresh_time_sensitive']
    
    if evergreen_ages:
        print(f"   Evergreen posts: min={min(evergreen_ages):.1f} months, max={max(evergreen_ages):.1f} months, avg={sum(evergreen_ages)/len(evergreen_ages):.1f} months")
    if fresh_ages:
        print(f"   Fresh posts: min={min(fresh_ages)} days, max={max(fresh_ages)} days, avg={sum(fresh_ages)/len(fresh_ages):.1f} days")
    ...
    if false_evergreen:
        ...
        for i, item in enumerate(false_evergreen[:5]):
            ...
            print(f"      Urgency: {post['urgency_score'] or 0.0:.2f}, Category: {post['category'] or 'N/A'}")
            if item['age']:
                print(f"      Age: {item['age']['days']} days ({item['age']['months']:.1f} months)")
            print()
    
    # Recommendations
    print(f"\n💡 RECOMMENDATIONS:")
    print(f"   1. ❌ EXCLUDE false evergreen posts (high urgency, time_sensitive flag, NEWS category, old)")
    print(f"   2. ❌ EXCLUDE time-sensitive posts from last month (all deprecated)")
    print(f"   3. ✅ INCLUDE only truly evergreen posts (<6 months old, low urgency, not time-sensitive)")
    print(f"   4. ✅ INCLUDE fresh time-sensitive posts (≤7 days)")
    print(f"   5. 🔍 CONSIDER: Only include evergreen posts < 3 months old for higher quality")
