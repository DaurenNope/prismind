...
class ProductionPublisher:
    ...
    async def process_batch(
        self, limit: int = 20, platform_filter: str = None, dry_run: bool = False
    ) -> Dict[str, Any]:
        """
            ...
            try:
                ...
                if scheduled_post:
                    ...
                    results["by_platform"][platform] = (
                        results["by_platform"].get(platform, 0) + 1
                    )
                else:
                    results["errors"] += 1
    ...
    async def process_single_post(
        self, post: Dict[str, Any], dry_run: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        analyzed = await self.analyzer.analyze_for_rewriting(
            ...
            metadata={"platform": source_platform, "url": url, "title": title},
        )
        ...
        logger.info(
            f"   Time sensitivity: {analyzed.get('time_sensitivity', 'unknown')}"
        )
        ...
        decision = self.scheduler.schedule_rewritten_post(
            rewritten_content=rewritten, platform_override=output_platform
        )

        # Prepare scheduled post data
        scheduled_post = {
            "id": f"{output_platform}_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            "source_post": {"title": title, "url": url, "platform": source_platform},
        ...
        if not dry_run:
            ...
            logger.info(
                f"💾 [DRY RUN] Would save to: {self.schedule_dir}/{scheduled_post['id']}.json"
            )
...
async def main():
    ...
    parser.add_argument(
        "--limit", type=int, default=10, help="Number of posts to process"
    )
    parser.add_argument(
        "--platform",
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Dry run (don't save files)"
    )
    parser.add_argument(
        "--show-schedule", action="store_true", help="Show scheduled posts overview"
    )
    ...
    results = await publisher.process_batch(
        limit=args.limit, platform_filter=args.platform, dry_run=args.dry_run
    )
