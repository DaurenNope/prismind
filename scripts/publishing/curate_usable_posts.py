...
def is_post_usable(
    post: Dict[str, Any], now: Optional[datetime] = None
    ...
    if not analysis_model or analysis_model not in (
        "gemini",
        "mistral",
        "ollama",
        "qwen",
    ...
    if relevance_window == "evergreen":
        ...
        if check_time_sensitive_keywords(content, ai_summary):
            is_false_evergreen = True
            logger.debug(
                f"Post {post.get('post_id')} marked as false evergreen due to time-sensitive keywords"
            )
...
def get_usable_posts_from_sqlite(
    db_path: str = "beyondlines.db",
) -> List[Dict[str, Any]]:
    ...
    posts = cur.fetchall()
    usable_posts = []
    excluded_posts = {"content_quality": 0, "time_sensitive_keywords": 0, "other": 0}
    now = datetime.now(timezone.utc)
    ...
    for row in posts:
        ...
        if is_usable:
            ...
            if inclusion_reason is None:
                ...
                ai_summary = post.get("ai_summary", "") or ""
                if (
                    check_time_sensitive_keywords(content, ai_summary)
                    and post.get("relevance_window") == "evergreen"
                ):
...
def sync_usable_posts_to_supabase(
    usable_posts: List[Dict[str, Any]], dry_run: bool = False
) -> Dict[str, Any]:
    ...
    for i in range(0, len(usable_posts), batch_size):
        ...
        for post in batch:
            try:
                ...
                best_persona_reasons = parse_array_field(
        ...
        if batch_data:
            try:
                ...
                logger.info(
                    f"Inserted batch {i//batch_size + 1}: {len(batch_data)} posts"
                )
            except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Dry run (do not sync to Supabase)"
    )
    parser.add_argument(
        "--db-path", default="beyondlines.db", help="Path to SQLite database"
    )
    ...
    if total_analyzed > 0:
        ...
        logger.info(
            f"  Usable: {len(usable_posts)} ({len(usable_posts)/total_analyzed*100:.1f}%)"
        )
        logger.info(
            f"  Excluded - Content quality issues: {excluded_posts['content_quality']} ({excluded_posts['content_quality']/total_analyzed*100:.1f}%)"
        )
        logger.info(
            f"  Excluded - Time-sensitive keywords: {excluded_posts['time_sensitive_keywords']} ({excluded_posts['time_sensitive_keywords']/total_analyzed*100:.1f}%)"
        )
        logger.info(
            f"  Excluded - Other reasons: {excluded_posts['other']} ({excluded_posts['other']/total_analyzed*100:.1f}%)"
        )
    ...
    if not args.dry_run:
        ...
        if result["success"]:
            logger.info(
                f"✅ Successfully synced {result['stats']['inserted']} usable posts to Supabase"
            )
        else:
