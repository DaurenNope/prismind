...
def backfill_post_fields(post_data: dict) -> dict:
    ...
    if not post_data.get('rewrite_readiness'):
        ...
        post_data['rewrite_readiness'] = 'ready' if summary_len >= 150 else ('needs_context' if summary_len < 80 else 'needs_trim')
        updated = True
    ...
    if post_data.get('rewrite_reasons') is None:
        ...
        post_data['rewrite_reasons'] = [f"Good tags ({tags_count})", f"Concepts ({concepts_count})"]
        updated = True
    ...
    if post_data.get('needs_deep_analysis') is None:
        ...
        post_data['needs_deep_analysis'] = True if rewrite_score >= 8 and confidence < 0.8 else False
        updated = True
...
def backfill_sqlite_posts(limit=None):
    ...
    for row in rows:
        ...
        for field in ['tags', 'key_concepts', 'rewrite_reasons', 'rewrite_risks', 'best_persona_reasons', 
                      'persona_fit_scores', 'persona_fit_reasons', 'hashtags', 'mentions']:
            ...
            try:
                ...
                if result:
                    ...
                    if updated_count % 50 == 0:
                        logger.info(f"   Synced {updated_count}/{len(rows)} posts to Supabase...")
            except Exception as e:
...
def main():
    ...
    logger.info(f"✅ Backfill complete: {sqlite_count} SQLite, {supabase_count} Supabase")
