...
from scripts.curate_usable_posts import check_content_quality, check_time_sensitive_keywords, parse_datetime
from src.utils.logging_config import get_logger

logger = get_logger(__name__)


def validate_usable_posts(db_path: str = 'prismind.db') -> Dict[str, Any]:
...
def main():
    ...
    print(f"   ✅ Passing all checks: {results['passing_all_checks']} ({results['passing_all_checks']/results['total_analyzed']*100:.1f}%)")
    print(f"   ❌ Failing content quality: {results['failing_content_quality']}")
    print(f"   ❌ Failing time-sensitive keywords: {results['failing_time_sensitive_keywords']}")
    ...
    if results['issues']:
        ...
        for issue in results['issues'][:10]:
            print(f"   {issue['post_id']} ({issue['platform']}): {issue['issue']} - {issue['error']}")
    ...
    if len(usable_posts) == results['passing_all_checks']:
        ...
        print(f"   ⚠️  MISMATCH! Difference: {abs(len(usable_posts) - results['passing_all_checks'])}")
