        ...
        for link in unique_links[:limit]:
            ...
            try:
                ...
                if post and post.content:
                    ...
                    logging.info(f"✅ Scraped post #{len(posts_scraped)}: {post.content[:100]}...")
                else:
...
async def update_qronoya_examples(posts: list):
    ...
    for i, post in enumerate(posts[:15]):  # Take first 15
        if i < len(data['examples']):
            ...
            data['examples'][i]['notes'] = f"Real post from @qronoya - {post.get('created_at', 'unknown date')}"
            data['examples'][i]['why_good_example'] = "Authentic voice from actual Threads post"
