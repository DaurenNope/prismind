...
class DatabaseReviewWorkflow:
    ...
    async def review_single_post(self, post: Dict) -> str:
        ...
        print(f"\n📝 ORIGINAL POST:")
        print("-" * 100)
        print(f"Source: {source_platform} → Output: {output_platform} ({language.upper()})")
        ...
        if choice == '1':
            # Approve
            return await self.handle_approval(post, gemini_output, output_platform, language)

        elif choice == '2':
            # Edit
            return await self.handle_correction(post, gemini_output, output_platform, language)
    ...
    async def handle_approval(self, post: Dict, rewrite: str, platform: str, language: str) -> str:
    ...
    async def handle_correction(self, post: Dict, gemini_output: str, platform: str, language: str) -> str:
        ...
        # Log correction
        self.log_correction(post, gemini_output, your_correction, differences, platform, language)
    ...
    def log_correction(self, post: Dict, gemini_output: str, your_correction: str, differences: dict, platform: str, language: str):
    ...
    def show_session_summary(self, stats: dict):
        ...
        print()
        print(f"🚀 Next steps:")
        print(f"  • Continue reviewing: python review_and_publish_from_database.py --limit 10")
        print(f"  • After 10-20 reviews: python scripts/update_voice_model.py")
        print(f"  • View scheduled posts: python review_and_publish_from_database.py --show-schedule")
        print("="*100)
...
async def main():
    ...
    parser.add_argument('--limit', type=int, default=5, help='Number of posts to review (default: 5)')
    parser.add_argument('--platform', choices=['threads', 'twitter', 'reddit', 'telegram'], help='Filter by platform')
    parser.add_argument('--show-schedule', action='store_true', help='Show scheduled posts')
    ...
    await workflow.start_review_session(
        limit=args.limit,
