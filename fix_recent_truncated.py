...
async def main():
    ...
    result = sm.client.table('posts').select('post_id, author, content, url, created_at').eq('platform', 'twitter').gte('created_at', one_month_ago).execute()
    ...
    print()
    print(f'🎉 Success rate: {fixed}/{len(truncated)} ({fixed*100//len(truncated) if truncated else 0}%)')
