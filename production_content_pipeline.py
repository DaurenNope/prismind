...
async def schedule_post(profile_key: str, content: str, platform: str, post_time: datetime, source_post_id: str = None):
    """
    ...
    try:
        ...
        print(f"✅ Scheduled {platform} post for {profile_key} at {post_time.strftime('%H:%M')}")
        return result.data
...
async def generate_schedule_for_profile(profile_key: str):
    ...
    for i, (content_type, post_time) in enumerate(posting_times):
        ...
        for platform in platforms:
            await schedule_post(profile_key, rewritten, platform, post_time, source_post_id=post.get('id'))
            scheduled_count += 1
