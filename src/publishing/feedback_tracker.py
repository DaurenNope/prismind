...
class FeedbackTracker:
    ...
    def _ensure_table_exists(self) -> None:
        ...
        if not self._service_key:
            logger.warning("SUPABASE_SERVICE_ROLE_KEY not set; cannot auto-create rewrite_feedback table")
            return
        ...
        """
        notify = "select pg_notify('pgrst', 'reload schema');"
        try:
            admin_client = create_client(self._supabase_url, self._service_key)
            ddl_result = admin_client.rpc("pg_execute_sql", {"query": ddl}).execute()
            if getattr(ddl_result, "error", None):
                logger.warning("Could not ensure rewrite_feedback table: %s", ddl_result.error)
                return
            admin_client.rpc("pg_execute_sql", {"query": notify}).execute()
            logger.info("✅ rewrite_feedback table ensured and schema cache refreshed")
        except Exception as exc:
            logger.warning("Failed to ensure rewrite_feedback table: %s", exc)
    ...
    async def store_feedback(
        ...
        try:
            ...
            response = self.supabase.table('rewrite_feedback').insert(data).execute()

            if response.data and len(response.data) > 0:
                record_id = response.data[0]['id']
                logger.info(f"✅ Stored feedback for rewrite {rewrite_id}: {rating} ({feedback_type})")
                return record_id
            else:
            ...
            if "PGRST205" in str(e):
                self._ensure_table_exists()
                logger.warning("rewrite_feedback table missing; ensured and skipping store this time")
                return None
            logger.error(f"❌ Error storing feedback: {e}")
            return None

    async def get_rewrite_feedback(
        self,
        rewrite_id: str
    ) -> List[Dict[str, Any]]:
        """
        ...
        """
        try:
            response = self.supabase.table('rewrite_feedback')\
                .select('*')\
    ...
    async def get_feedback_stats(
        ...
        try:
            ...
            if persona or platform:
                logger.warning("Persona/platform filtering on metadata not yet implemented")
            ...
            # Rating stats
            ratings = [f['rating'] for f in feedback_records if f.get('rating') is not None]
            avg_rating = sum(ratings) / len(ratings) if ratings else 0

            # Approval rate (for thumbs feedback)
            thumbs_feedback = [f for f in feedback_records if f.get('feedback_type') == 'thumbs']
            ...
            approval_rate = (approvals / len(thumbs_feedback) * 100) if thumbs_feedback else 0
            ...
            logger.info(f"📊 Feedback stats: {total_count} records, avg rating {avg_rating:.2f}, approval rate {approval_rate:.1f}%")
    ...
    async def get_approved_rewrites(
        ...
        try:
            ...
            logger.info(f"🏆 Found {len(records)} highly-rated rewrites (rating >= {min_rating})")
    ...
    async def get_rejected_rewrites(
        ...
        try:
            ...
            logger.info(f"⚠️ Found {len(records)} low-rated rewrites (rating <= {max_rating})")
    ...
    async def learn_from_feedback(
        ...
        try:
            ...
            approved = await self.get_approved_rewrites(persona=persona, platform=platform)
            rejected = await self.get_rejected_rewrites(persona=persona, platform=platform)
            ...
            logger.info(f"🧠 Learned from {len(approved)} approved and {len(rejected)} rejected rewrites")
            ...
            return {
                ...
                'suggestions': self._generate_suggestions(approved_patterns, rejected_patterns)
            }
    ...
    def _analyze_patterns(self, feedback_records: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
    ...
    def _generate_suggestions(
        ...
        if approved_len and rejected_len:
            if approved_len < rejected_len * 0.8:
                suggestions.append(f"Users prefer shorter content (~{approved_len} chars vs {rejected_len} chars)")
            elif approved_len > rejected_len * 1.2:
                suggestions.append(f"Users prefer longer content (~{approved_len} chars vs {rejected_len} chars)")
        ...
        if approved_emoji is not None and rejected_emoji is not None:
            if approved_emoji > rejected_emoji * 1.5:
                suggestions.append(f"Users prefer more emojis ({approved_emoji:.0%} vs {rejected_emoji:.0%})")
            elif approved_emoji < rejected_emoji * 0.5:
                suggestions.append(f"Users prefer fewer emojis ({approved_emoji:.0%} vs {rejected_emoji:.0%})")

        # Content type preferences
        approved_types = approved_patterns.get('content_types', {})
        if approved_types:
            top_type = max(approved_types.items(), key=lambda x: x[1])
            suggestions.append(f"Most approved content type: {top_type[0]} ({top_type[1]} occurrences)")
