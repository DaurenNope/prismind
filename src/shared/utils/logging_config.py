"""
BEYONDLINES Centralized Logging Configuration
======================================

Provides consistent logging setup across all BEYONDLINES modules.
...
class BEYONDLINESFormatter(logging.Formatter):
    """Custom formatter for BEYONDLINES with structured output"""
...
class StructuredLogger:
    ...
    def _setup_logger(self, level: str):
        ...
        console_handler.setFormatter(BEYONDLINESFormatter())
    ...
    def _log(self, level: int, message: str, **kwargs):
        ...
        # Downgrade known benign duplicate errors to INFO
        try:
            normalized_message = str(message).lower()
            if any(
                token in normalized_message
                for token in ("posts_url_key", "23505", "duplicate key value")
            ):
                level = logging.INFO
        except Exception:
            pass
...
class PerformanceLogger:
    ...
    def __init__(self, name: str = "performance"):
        self.logger = logging.getLogger(f"beyondlines.{name}")
        self.metrics = {}
...
def get_logger(name: str, level: str = None) -> StructuredLogger:
    ...
    if name not in _loggers:
        ...
        _loggers[name] = StructuredLogger(f"beyondlines.{name}", log_level)
...
def setup_logging():
    """Setup logging for the entire BEYONDLINES application"""
    ...
    if log_format == "json":
        ...
        handler = logging.StreamHandler(sys.stdout)
        handler.setFormatter(BEYONDLINESFormatter())
        root_logger.addHandler(handler)
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            perf_logger.log_execution_time(
                func.__name__, duration, args_count=len(args), kwargs_count=len(kwargs)
            )
            return result
        except Exception as e:
            duration = (datetime.now() - start_time).total_seconds()
