...
class PostValidator:
    ...
    def validate_post(self, post_data: Dict[str, Any]) -> ValidationResult:
        ...
        if not cleaned_author or len(cleaned_author.strip()) < self.MIN_AUTHOR_LENGTH:
            ...
            if cleaned_handle and len(cleaned_handle.strip()) >= self.MIN_AUTHOR_LENGTH:
                ...
                if url and '@' in url:
                    ...
                    try:
                        ...
                        if len(url_parts) > 1:
                            ...
                            if potential_handle and len(potential_handle) >= self.MIN_AUTHOR_LENGTH:
                ...
                if not cleaned_author or len(cleaned_author.strip()) < self.MIN_AUTHOR_LENGTH:
                    post_id = post_data.get('post_id', '')[:20] if post_data.get('post_id') else ''
                    if post_id:
        ...
        # Validate content
        content_valid, content_errors = self._validate_content(post_data.get('content', ''))
        ...
        # Validate platform
        platform_valid, platform_errors = self._validate_platform(post_data.get('platform', ''))
    ...
    def _validate_content(self, content: str) -> Tuple[bool, List[str]]:
        ...
        if len(content.strip()) < self.MIN_CONTENT_LENGTH:
            errors.append(f"Content too short ({len(content)} chars, minimum {self.MIN_CONTENT_LENGTH})")
            return False, errors
        ...
        if len(words) >= 3:
            ...
            if repeat_count >= 3:
                errors.append(f"Content appears to be repeated username spam: '{first_word}'")
                return False, errors
    ...
    def validate_batch(self, posts: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        """
