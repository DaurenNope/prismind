...
def categorize_repo(data: Dict[str, Any]) -> str:
    ...
    if any(topic in ["ai", "ml", "machine-learning", "deep-learning", "llm"] for topic in topics):
        return "AI/ML"
    elif any(topic in ["web", "frontend", "react", "vue", "angular"] for topic in topics):
...
def generate_why_matters(data: Dict[str, Any]) -> str:
    ...
    # Innovation
    if any(word in description.lower() for word in ["first", "revolutionary", "breakthrough", "innovative"]):
        reasons.append("innovative approach")
    
    # Industry adoption
    if any(word in description.lower() for word in ["production", "enterprise", "industry"]):
...
def test_github_metadata():
    ...
    print("🧪 Testing GitHub Metadata Extraction\n")
