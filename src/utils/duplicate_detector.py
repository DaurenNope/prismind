        ...
        if self.db:
            try:
                ...
                # Check for StorageFacade (has get_posts)
                    ...
                    all_posts = self.db.get_posts(
                        limit=10000
                    )  # Reasonable limit for duplicate detection
                else:
                    logger.warning(
                        f"Database manager {type(self.db)} has no get_all_posts or get_posts method"
                    )
                    return

                if all_posts:
                    logger.info(
                        f"Loading {len(all_posts)} posts into duplicate detection cache..."
                    )
                    ...
                    logger.info(
                        f"✅ Loaded {len(self._url_cache)} URLs and {len(self._content_hash_cache)} content hashes into cache"
                    )
                else:
                    logger.warning(
                        f"Database manager doesn't have get_all_posts() or get_posts() method - skipping cache initialization"
                    )
                ...
                if self.db:
                    try:
                        ...
                        if hasattr(self.db, "get_posts_by_platform"):
                            threads_posts = self.db.get_posts_by_platform(
                                "threads", limit=None
                            )
                        elif hasattr(self.db, "get_posts"):
                        ...
                        for post in threads_posts:
                            ...
                            if existing_url:
                                ...
                                if (
                                    existing_match
                                    and existing_match.group(1) == post_id
                                ):
                                    return True
                    except Exception as e:
        ...
        if self.db:
            try:
                ...
                for post in all_posts:
                    if (
                        post.get("url")
                        and self.normalize_url(post["url"]) == normalized
                    ):
                        self._url_cache.add(normalized)  # Add to cache
                        return True
            except Exception as e:
...
def test_duplicate_detector():
    ...
    logger.info("1️⃣  URL Normalization:")
    logger.info("-" * 70)
    ...
    test_urls = [
        ...
        (
            "https://github.com/user/repo?utm_source=twitter",
    ...
    for url1, url2 in test_urls:
        ...
        logger.info(f"{match}")
        logger.info(f"  URL 1: {url1}")
        logger.info(f"  URL 2: {url2}")
        logger.info(f"  Norm:  {norm1}")
        logger.info()

    # Test content hashing
    logger.info("2️⃣  Content Hashing:")
    logger.info("-" * 70)
    ...
    logger.info(f"Content 1: {content1}")
    logger.info(f"Content 2: {content2}")
    logger.error(f"Hash match: {'✅ SAME' if hash1 == hash2 else '❌ DIFFERENT'}")
    logger.info()
    logger.info(f"Content 3: {content3}")
    logger.error(
        f"Hash match: {'✅ SAME' if hash1 == hash3 else '❌ DIFFERENT (expected)'}"
    )
    logger.info()

    # Test duplicate detection
    logger.info("3️⃣  Duplicate Detection:")
    logger.info("-" * 70)
    ...
    logger.info(f"Item 1: {'Duplicate' if is_dup else 'Unique'}")

    detector.mark_as_seen(item1)

    is_dup = detector.is_duplicate(item2)
    logger.info(
        f"Item 2 (same URL): {'✅ Duplicate' if is_dup else '❌ Should be duplicate!'}"
    )
    logger.info()
    ...
    logger.info(
        f"📊 Stats: {stats['cached_urls']} URLs, {stats['cached_content_hashes']} content hashes"
    )
    logger.info()
    logger.info("✅ Duplicate detector working!")
