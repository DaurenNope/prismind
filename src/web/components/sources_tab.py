...
def load_sources_config() -> Dict:
    ...
    return {
        "telegram_channels": {"technical": [], "builder": [], "trendsetter": []},
        "rss_feeds": {"technical": [], "builder": [], "trendsetter": []},
    }
...
def render_sources_tab():
    ...
    st.info(
        f"**Target Niche:** {persona_meta['niche']}  \n**Writing Style:** {persona_meta['style']}"
    )
    ...
    with source_type[0]:
        ...
        if telegram_channels:
            ...
            st.info(
                f"No Telegram channels configured for {personas[selected_persona]['name']} yet."
            )
        ...
        if st.button("➕ Add Channel", key=f"add_tg_{selected_persona}"):
            if new_channel:
                ...
                existing = [
                    ...
                    else ch.get("username", ch.get("channel", ""))
                ...
                if new_channel.strip() not in existing:
                    ...
                    channel_data = {
                        ...
                        "niche": new_channel_niche.strip()
                        if new_channel_niche.strip()
                        else persona_meta["niche"],
                    ...
                    save_sources_config(config)
                    st.success(
                        f"Added @{new_channel} targeting {channel_data['niche']}"
                    )
                    st.rerun()
                else:
        ...
        for i, (channel, desc) in enumerate(persona_suggestions):
            existing = [
                ch if isinstance(ch, str) else ch.get("username", ch.get("channel", ""))
                for ch in telegram_channels
            ]
            ...
            with cols[i % 2]:
                if channel not in existing:
                    if st.button(
                        f"➕ @{channel}", key=f"suggest_tg_{selected_persona}_{channel}"
                    ):
                        channel_data = {
                            "username": channel,
                        ...
                        telegram_channels.append(channel_data)
                        config["telegram_channels"][selected_persona] = (
                            telegram_channels
                        )
                        save_sources_config(config)
                        st.rerun()
    ...
    with source_type[1]:
        ...
        if rss_feeds:
            ...
            st.info(
                f"No RSS feeds configured for {personas[selected_persona]['name']} yet."
            )
                ...
                with st.spinner("Collecting from all channels..."):
                    try:
                        ...
                        async def collect():
                            ...
                            for persona_key, persona_channels in config[
                                "telegram_channels"
                            ].items():
                                for channel_data in persona_channels:
                                    if isinstance(channel_data, str):
                                        channel = channel_data
                                        niche = get_persona_metadata(persona_key)[
                                            "niche"
                                        ]
                                    else:
                                        channel = channel_data.get(
                                        ...
                                        niche = channel_data.get(
                                            "niche",
                                            get_persona_metadata(persona_key)["niche"],
                                        )
                                    ...
                                    try:
                                        ...
                                        if not existing.data:
                                            post_data = {
                                                ...
                                                "metadata": json.dumps(
                                                    {
                                                        "source_persona": channel_meta.get(
                                                            "persona"
                                                        ),
                                                        "target_niche": channel_meta.get(
                                                            "niche"
                                                        ),
                                            ...
                                            supabase.client.table("posts").insert(
                                                post_data
                                            ).execute()
                                            saved += 1
                                    except:
                        ...
                        st.success(
                            f"✅ Collected {total} messages, saved {saved} new posts!"
                        )
...
TELEGRAM_API_HASH=your_api_hash
        ...
        st.code(
            "0 */4 * * * cd /path/to/prismind && .venv311/bin/python collect_telegram_channels.py"
        )
