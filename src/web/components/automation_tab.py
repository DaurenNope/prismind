...
def render_automation_tab():
    ...
    if st.session_state.automation and st.session_state.automation_running:
        ...
        st.info(
            "⚪ Scheduler not running. Click 'Start Scheduler' to begin autonomous discovery."
        )
    ...
    if st.button("Create List"):
        ...
        with st.spinner(f"Creating {list_type} reading list..."):
            ...
            filtered = [
                ...
                and any(kw in p.content.lower() for kw in keywords)
            ]

            # Create list
            reading_list = asyncio.run(
                librarian.create_reading_list(
                    list_type, filtered[:20], f"Curated {list_type.lower()} content"
                )
            )

            st.success(
                f"✅ Created '{reading_list['name']}' with {reading_list['progress']['total']} items!"
            )
