    ...
    try:
        ...
        with col1:
            status_filter = st.selectbox(
                "Status", ["all", "new", "shown", "bookmarked", "dismissed"], index=0
            )

        with col2:
            source_filter = st.selectbox(
                "Source", ["all", "rss", "github", "reddit"], index=0
            )
        ...
        if not discoveries:
            st.info(
                "No discoveries found. Run `/autonomous discover` in Telegram to start!"
            )
            return
        ...
        with col3:
            avg_quality = (
                sum(d.get("quality_score", 0) for d in discoveries) / len(discoveries)
                if discoveries
                else 0
            )
...
def render_discovery_card(discovery: Dict[str, Any], supabase, idx: int = 0):
    ...
    with st.expander(
        ...
        with col1:
            st.markdown(
                f"**Source:** {discovery.get('platform', 'unknown')} / {discovery.get('source', 'unknown')}"
            )
...
def mark_as_shown(discovery_id: str, supabase):
    ...
    try:
        supabase.client.table("discoveries").update(
            {"status": "shown", "shown_to_user_at": datetime.now().isoformat()}
        ).eq("id", discovery_id).execute()
    except Exception as e:
