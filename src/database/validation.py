...
class DatabaseValidation:
    ...
    def validate_and_monitor_post(self, post: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            record_fn = getattr(self, '_record_post_operation', None) or self._record_post_operation
            if record_fn:
                try:
                    record_fn(
    ...
    def normalize_post_data(self, post: Dict[str, Any]) -> Dict[str, Any]:
        ...
        def normalize_boolean(value, default=True):
            ...
            if isinstance(value, str):
                ...
                if value_stripped.startswith(('{', '[')):
                    try:
                        from src.utils.logging_config import get_logger
                        get_logger(__name__).warning(f"Invalid boolean value appears to be JSON: {value_stripped[:50]}..., using default {default}")
                    except:
                        pass
                    return default
                ...
                try:
                    from src.utils.logging_config import get_logger
                    get_logger(__name__).warning(f"Unknown boolean string value: {value_stripped[:50]}..., using default {default}")
                except:
                    pass
                return default
            ...
            try:
                from src.utils.logging_config import get_logger
                get_logger(__name__).warning(f"Unknown boolean type: {type(value)}, using default {default}")
            except:
                pass
            return default
        ...
        if created_at and isinstance(created_at, str) and len(created_at) > 50:
            ...
            fallback = normalized.get('created_timestamp') or normalized.get('saved_at') or normalized.get('updated_timestamp')
        ...
        if analyzed_at:
            ...
            if isinstance(analyzed_at, (list, dict)):
                ...
                logger.warning(f"Corrupted analyzed_at is {type(analyzed_at).__name__}, fixing to current timestamp")
                ...
                invalid_values = ['mistral', 'gemini', 'ollama', 'qwen', 'ai_service', 'unknown', 'none', 'null', '']
                if analyzed_at_lower in invalid_values:
                    # It's a service name - corrupted
                    logger.warning(f"Corrupted analyzed_at is service name '{analyzed_at}', fixing to current timestamp")
                    normalized['analyzed_at'] = datetime.utcnow().isoformat()
                elif not analyzed_at.startswith(('202', '2023', '2024', '2025')) and len(analyzed_at) < 10:
                    # Doesn't look like a timestamp (should start with year and be at least 10 chars)
                    logger.warning(f"Corrupted analyzed_at doesn't look like timestamp: '{analyzed_at[:50]}', fixing to current timestamp")
                    normalized['analyzed_at'] = datetime.utcnow().isoformat()
                else:
                    # Try to normalize it as a timestamp
                    normalized['analyzed_at'] = normalize_timestamp(analyzed_at, datetime.utcnow().isoformat())
            else:
        ...
        currently_deprecated = (current_category and str(current_category).upper() == 'DEPRECATED')
        created_at = normalized.get('created_at')
            ...
            try:
                ...
                if isinstance(created_at, str):
                    ...
                    if created_dt:
                        ...
                        if relevance_window and relevance_window != 'evergreen' and relevance_window != '':
        ...
        if not category or category not in valid_categories:
            ...
            if not category or category not in ['TECH', 'DATING', 'CRYPTO', 'BUSINESS', 'LEARNING', 'NEWS', 'PERSONAL', 'HEALTH', 'ENTERTAINMENT', 'OTHER']:
                if content_lower:
                    if any(kw in content_lower for kw in ['crypto', 'bitcoin', 'blockchain', 'defi', 'nft', 'ethereum', 'web3', 'token', 'solana', 'eth', 'btc']):
                        normalized['category'] = 'CRYPTO'
                    elif any(kw in content_lower for kw in ['dating', 'relationship', 'romance', 'love', 'partner', 'single', 'marriage', 'heart', 'broken']):
                        normalized['category'] = 'DATING'
                    elif any(kw in content_lower for kw in ['ai', 'code', 'programming', 'software', 'tech', 'developer', 'engineering', 'algorithm', 'api', 'framework', 'github', 'python', 'javascript']):
                        normalized['category'] = 'TECH'
                    elif any(kw in content_lower for kw in ['startup', 'business', 'entrepreneur', 'marketing', 'finance', 'invest', 'revenue', 'funding', 'vc', 'venture']):
                        normalized['category'] = 'BUSINESS'
                    elif any(kw in content_lower for kw in ['learn', 'tutorial', 'how to', 'guide', 'education', 'course', 'study', 'teaching']):
                        normalized['category'] = 'LEARNING'
                    elif any(kw in content_lower for kw in ['news', 'breaking', 'announced', 'reported', 'politics', 'world', 'government']):
                        normalized['category'] = 'NEWS'
                    elif any(kw in content_lower for kw in ['health', 'fitness', 'nutrition', 'wellness', 'medical', 'diet', 'exercise', 'doctor']):
                        normalized['category'] = 'HEALTH'
                    elif any(kw in content_lower for kw in ['movie', 'music', 'game', 'entertainment', 'fun', 'meme', 'comedy', 'film']):
        ...
        invalid_persona_values = ['evergreen', 'timely', 'trending', 'breaking', 'urgent', 
                                 'this-week', 'this-month', 'this-year', 'next-week', 'next-month',
                                 'same-day', '24-72h', 'this-week', 'this-month']
        ...
        if normalized.get('category') and not normalized.get('best_persona_key'):
            ...
            if category in category_to_persona:
                ...
                normalized['best_persona_score'] = 7.0  # Medium confidence for category-based mapping
        
        # Fix string scores (should be numeric)
        for field in ['rewrite_score', 'analysis_confidence', 'best_persona_score', 'value_score', 'quality_score', 'urgency_score']:
        ...
        if not normalized.get('rewrite_readiness'):
            ...
            normalized['rewrite_readiness'] = 'ready' if summary_len >= 150 else ('needs_context' if summary_len < 80 else 'needs_trim')
        ...
        if normalized.get('rewrite_reasons') is None:
            ...
            normalized['rewrite_reasons'] = [f"Good tags ({tags_count})", f"Concepts ({concepts_count})"]
        ...
        if normalized.get('needs_deep_analysis') is None:
            ...
            normalized['needs_deep_analysis'] = True if rewrite_score >= 8 and confidence < 0.8 else False
        ...
        if not normalized.get('best_persona_key') or normalized.get('best_persona_key') in invalid_persona_values:
            normalized['best_persona_key'] = None
        ...
        normalized['time_sensitive'] = normalize_boolean(normalized.get('time_sensitive'), False)
        normalized['needs_deep_analysis'] = normalize_boolean(normalized.get('needs_deep_analysis'), False)
