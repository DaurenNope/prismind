...
"""
Database Analysis Operations for BEYONDLINES
...
class DatabaseAnalysis:
    ...
    def update_rewrite_status(
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.commit()
                logger.info(f"✅ Updated rewrite status for post {post_id} to {status}")
                return True

        except Exception as e:
            logger.error(f"❌ Error updating rewrite status: {e}")
            return False
    ...
    def get_rewrite_candidates(self, limit: int = 50) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"❌ Error getting rewrite candidates: {e}")
            return []
    ...
    def get_high_quality_posts(
        ...
        try:
            ...
            logger.error(f"❌ Error getting high quality posts: {e}")
            return []
    ...
    def get_low_quality_posts(
        ...
        try:
            ...
            logger.error(f"❌ Error getting low quality posts: {e}")
            return []

    def get_unanalyzed_posts(
        self, limit: Optional[int] = 100, platforms: Optional[List[str]] = None
    ) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"❌ Error getting unanalyzed posts: {e}")
            return []
    ...
    def get_posts_by_score_range(
        ...
        try:
            ...
            logger.error(f"❌ Error getting posts by score range: {e}")
            return []
    ...
    def get_analysis_stats(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute(
                    "SELECT COUNT(*) FROM posts WHERE analysis_timestamp IS NOT NULL"
                )
                analyzed_count = cursor.fetchone()[0]

                # Total unanalyzed posts
                cursor.execute(
                    "SELECT COUNT(*) FROM posts WHERE analysis_timestamp IS NULL AND rewrite_status != 'deleted'"
                )
                unanalyzed_count = cursor.fetchone()[0]

                # Rewrite candidates
                cursor.execute(
                    "SELECT COUNT(*) FROM posts WHERE is_rewrite_candidate = TRUE"
                )
    ...
    def _create_insights_summary(
        self, insights: List[str], action_items: List[str]
    ) -> str:
