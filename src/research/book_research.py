...
class BookResearcher:
    ...
    async def search_books(self, query: str, max_results: int = 5) -> List[Book]:
        ...
        google_books, openlibrary_books = await asyncio.gather(
            google_books_task, openlibrary_task, return_exceptions=True
        )

        # Handle exceptions
        if isinstance(google_books, Exception):
            logger.error(f"Error searching Google Books: {google_books}")
            google_books = []

        if isinstance(openlibrary_books, Exception):
            logger.error(f"Error searching OpenLibrary: {openlibrary_books}")
            openlibrary_books = []
        ...
        for book in all_books:
            ...
            book_id = (
                book.title.lower(),
                tuple(sorted([a.lower() for a in book.authors])),
            )
...
async def main():
    ...
    logger.info("Book Research Demo")
    logger.info("=" * 20)
    ...
    logger.info(f"Searching for books on: {query}\n")
    ...
    if books:
        ...
        for i, book in enumerate(books, 1):
            ...
            if book.published_date:
                logger.info(f"   Published: {book.published_date}")
            if book.publisher:
                logger.info(f"   Publisher: {book.publisher}")
            if book.isbn:
                logger.info(f"   ISBN: {book.isbn}")
            if book.pages:
                logger.info(f"   Pages: {book.pages}")
