...
"""
Book Content Analyzer for BEYONDLINES
...
class BookContentAnalyzer:
    ...
    async def extract_relevant_sections(
        self, book_id: str, keywords: List[str], book_entry: BookEntry
    ) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"❌ Error extracting sections from book {book_id}: {e}")
            return []
    ...
    async def search_book_content(
        self, book_entry: BookEntry, search_query: str
    ) -> List[Dict]:
    ...
    def _read_file_content(self, file_path: str) -> str:
        ...
        try:
            ...
            logger.error(f"⚠️ Error reading file {file_path}: {e}")
            return ""
    ...
    def _search_content(self, content: str, query: str) -> List[Dict]:
        ...
        while True:
            ...
            highlighted_context = context.replace(
                content[pos : pos + len(query)], f"**{content[pos:pos + len(query)]}**"
            )
