...
class GitHubResearchAgent:
    ...
    async def research_repo(self, repo_url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            report = {
                ...
                # Rewrite angles
                "rewrite_angles": self._generate_rewrite_angles(metadata, readme, assessment)
            }
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                analysis = {
                    ...
                    "has_installation": bool(re.search(r'##?\s*(Install|Installation|Setup)', readme_text, re.I)),
                    "has_usage": bool(re.search(r'##?\s*(Usage|Getting Started|Quick Start)', readme_text, re.I)),
                    "has_examples": bool(re.search(r'##?\s*(Example|Examples|Demo)', readme_text, re.I)),
                    "has_docs": bool(re.search(r'##?\s*(Documentation|Docs)', readme_text, re.I)),
                    "has_contributing": bool(re.search(r'##?\s*(Contributing|Development)', readme_text, re.I)),
                ...
                code_match = re.search(r'```(?:python|javascript|js|typescript|ts)?\n(.*?)```', readme_text, re.S)
                if code_match:
    ...
    async def _analyze_structure(self, owner: str, repo: str) -> Dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                structure = {
                    "has_tests": any(item['name'] in ['tests', 'test', '__tests__', 'spec'] for item in contents if item['type'] == 'dir'),
                    "has_docs": any(item['name'] in ['docs', 'documentation'] for item in contents if item['type'] == 'dir'),
                    "has_examples": any(item['name'] in ['examples', 'demos', 'sample'] for item in contents if item['type'] == 'dir'),
                    "has_ci": any(item['name'] == '.github' for item in contents if item['type'] == 'dir'),
                    "has_package_json": any(item['name'] == 'package.json' for item in contents),
                    "has_requirements": any(item['name'] in ['requirements.txt', 'setup.py', 'pyproject.toml'] for item in contents),
                    "has_dockerfile": any(item['name'] in ['Dockerfile', 'docker-compose.yml'] for item in contents),
                    "has_license": any('license' in item['name'].lower() for item in contents),
    ...
    async def _analyze_community(self, owner: str, repo: str) -> Dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                # Calculate metrics
                recent_commits = len([c for c in commits if c]) if isinstance(commits, list) else 0
                
                closed_issues = len([i for i in issues if isinstance(i, dict) and i.get('state') == 'closed'])
                ...
                return {
                    ...
                    "community_score": self._calculate_community_score(recent_commits, len(contributors) if isinstance(contributors, list) else 0, issue_close_rate)
                }
    ...
    def _calculate_community_score(self, commits: int, contributors: int, close_rate: float) -> float:
    ...
    async def _discover_ecosystem(self, owner: str, repo: str, metadata: Dict) -> Dict[str, List[str]]:
    ...
    def _generate_usage_guide(self, readme: Dict, metadata: Dict) -> Dict[str, Any]:
        ...
        if language == "python":
            ...
            guide["install_command"] = f"go get github.com/{metadata.get('full_name', '')}"
        
        return guide
    
    def _assess_quality(self, metadata: Dict, community: Dict, readme: Dict) -> Dict[str, Any]:
        ...
        overall_score = (popularity_score * 0.3 + documentation_score * 0.4 + maintenance_score * 0.3)
    ...
    def _identify_target_audience(self, metadata: Dict, readme: Dict) -> List[str]:
        ...
        # Developer types
        if any(term in description for term in ["beginner", "getting started", "tutorial"]):
    ...
    def _generate_rewrite_angles(self, metadata: Dict, readme: Dict, assessment: Dict) -> List[Dict[str, str]]:
...
async def test_agent():
    ...
    print("🧪 Testing GitHub Research Agent\n")
    ...
    print()
    print(f"TLDR: {report['tldr']}")
    print()
    print(f"Quality Assessment:")
    print(f"  • Overall: {report['quality_assessment']['overall_score']}/10")
