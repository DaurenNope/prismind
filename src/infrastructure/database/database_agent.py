...
def get_database_agent() -> DatabaseAgent:
    ...
    The agent will be initialized asynchronously if not already initialized.
    For synchronous code, the agent will be returned immediately but may not
    be fully initialized until the async initialization completes.
    
    Returns:
        DatabaseAgent: Singleton instance of DatabaseAgent
    """
            ...
            if _database_agent_instance is None:
                ...
                try:
                    ...
                    if loop.is_running():
                        ...
                        logger.debug("DatabaseAgent initialization scheduled asynchronously")
                    else:
                        # No running event loop, initialize synchronously
                        loop.run_until_complete(_database_agent_instance.initialize())
                        logger.debug("DatabaseAgent initialized synchronously")
                except RuntimeError:
                    # No event loop available, initialization will happen on first async call
                    logger.debug("No event loop available, DatabaseAgent will initialize on first async call")
    
    return _database_agent_instance


# Optional: Register with AgentRegistry if available
def _register_with_registry():
    ...
    try:
        ...
        logger.debug("DatabaseAgent registered with AgentRegistry")
    except Exception as e:
        # Registry might not be available, that's okay
        logger.debug(f"AgentRegistry not available: {e}")


# Auto-register on module import (lazy registration)
try:
