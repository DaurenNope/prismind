        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if include_deleted:
                    ...
                    query = "SELECT * FROM posts WHERE deleted = 0 ORDER BY created_at DESC"
    ...
    def get_analytics(self, days: int = 14) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("SELECT COUNT(DISTINCT author) FROM posts WHERE deleted = 0 AND author IS NOT NULL")
                unique_authors = cursor.fetchone()[0] or 0

                cursor.execute("SELECT COUNT(DISTINCT platform) FROM posts WHERE deleted = 0")
                ...
                """, (f'-{max(days, 1)} day',))
                daily_posts = [{'date': row[0], 'count': row[1]} for row in cursor.fetchall()]
                ...
                cursor.execute("""
                    ...
                    ORDER BY DATE(created_at)
                """, (f'-{max(days, 1)} day',))
                score_trends = [{'date': row[0], 'average_score': row[1] or 0.0} for row in cursor.fetchall()]
                ...
                insights = []
                if total_posts == 0:
                    insights.append("No posts collected yet — run a collection to populate the dashboard.")
                else:
                    leading_platform = max(platform_distribution.items(), key=lambda item: item[1]['count'])[0] if platform_distribution else None
                    if leading_platform:
                        insights.append(f"{leading_platform.title()} currently has the most saved posts.")
                    if average_value_score:
                        insights.append(f"Average value score across posts is {average_value_score:.2f}.")
                    if top_authors:
                        top_author = top_authors[0]
                        insights.append(f"Top contributor: {top_author['author']} with {top_author['post_count']} posts.")
