...
"""
Health Monitor for BEYONDLINES
...
class HealthMonitor:
    ...
    def _check_components(self) -> Dict[str, Dict[str, Any]]:
        ...
        # Telegram Bot
        try:
            bot_pid_file = "/tmp/beyondlines_bot.pid"
        ...
        try:
            ...
            bookmarks = len(
                [
                    ...
                    and p.metadata.get("source") == "bookmark"
                ]
            )
            ...
            discovered = len(
                [
                    ...
                    if hasattr(p, "metadata")
                    and isinstance(p.metadata, dict)
                    and p.metadata.get("source") == "discovered"
                ]
            )
    ...
    def _get_recent_activity(self) -> Dict[str, Any]:
        ...
        return {
            "discoveries_today": len(
                [
                    d
                    for d in self.metrics_history["discoveries"]
            ...
            "errors_today": len(
                [
                    e
                    for e in self.metrics_history["errors"]
    ...
    def record_error(self, component: str, error: str):
        ...
        self.metrics_history["errors"].append(
            {"timestamp": datetime.now(), "component": component, "error": error}
        )
    ...
    def record_response_time(self, operation: str, duration: float):
        ...
        self.metrics_history["response_times"].append(
            {"timestamp": datetime.now(), "operation": operation, "duration": duration}
        )
    ...
    def get_alerts(self) -> List[Dict[str, Any]]:
        ...
        recent_discoveries = [
            d
            for d in self.metrics_history["discoveries"]
            if d.get("timestamp", datetime.min) > datetime.now() - timedelta(hours=4)
        ]
    ...
    def format_health_report(self, health: Dict[str, Any]) -> str:
        ...
        lines.append(
            f"Memory: {perf['memory']['used_gb']}/{perf['memory']['total_gb']} GB "
            f"({perf['memory']['percent']}%)"
        )
        lines.append(
            f"Disk: {perf['disk']['free_gb']} GB free ({perf['disk']['percent']}% used)\n"
        )
