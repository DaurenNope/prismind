#!/usr/bin/env python3
"""
Digest Generator for BEYONDLINES
...
class DigestGenerator:
    ...
    def generate_morning_digest(
        ...
        sorted_posts = sorted(
            scored_posts, key=lambda p: getattr(p, "quality_score", 0), reverse=True
        )
        ...
        logger.info(
            f"✅ Morning digest generated: {len(top_stories)} stories, "
    ...
    def generate_evening_summary(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"✅ Evening summary generated: {len(today_posts)} posts, "
    ...
    def _select_must_reads(self, posts: List[Any]) -> List[Dict[str, Any]]:
        ...
        for post in posts[:20]:  # Check top 20
            ...
            is_technical = any(
                word in content for word in ["code", "implementation", "architecture"]
            )
...
def test_digest_generator():
    ...
    logger.info(f"   Posts: {morning['posts_count']}")
    logger.info(f"   Stories: {len(morning['top_stories'])}")
    logger.info(f"   Topics: {len(morning['trending_topics'])}")

    # Format for Telegram
    telegram_text = generator.format_for_telegram(morning)
    logger.info(f"\n📱 Telegram format ({len(telegram_text)} chars):")
    logger.info(telegram_text[:500] + "...\n")

    # Test evening summary
    logger.info("🌆 Testing evening summary...")
    evening = generator.generate_evening_summary()
    logger.info(f"   Posts: {evening['total_posts']}")
    logger.info(f"   Quality: {evening['quality_posts']}")
    logger.info(f"   Platforms: {len(evening['platform_stats'])}")

    telegram_text = generator.format_for_telegram(evening)
    logger.info(f"\n📱 Telegram format ({len(telegram_text)} chars):")
    logger.info(telegram_text[:500] + "...\n")

    logger.info("✅ Digest generator working!")
