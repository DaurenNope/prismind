...
class DatabaseOperations:
    ...
    def _init_database(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_value_score ON posts(value_score)')
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_deleted ON posts(deleted)')
    ...
    def get_posts(self, limit: int = 100, offset: int = 0, platforms: List[str] = None, min_score: float = None, search_query: str = None) -> List[Dict]:
    ...
    def add_post(self, post_data: Dict[str, Any]) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                tags_source = post_data.get('tags') or post_data.get('suggested_tags', [])
                tags = self._serialize_json(tags_source)
                sentiment_analysis = self._serialize_json(post_data.get('sentiment_analysis', {}))
                content_summary = post_data.get('content_summary') or post_data.get('ai_summary') or post_data.get('summary')
                insights = self._serialize_json(post_data.get('insights', []))
                action_items = self._serialize_json(post_data.get('action_items', []))
                recommendations = self._serialize_json(post_data.get('recommendations', []))
                educational_value = post_data.get('educational_value')
                is_rewrite_candidate = 1 if post_data.get('is_rewrite_candidate', False) else 0
                ...
                columns = [
                    "post_id", "platform", "author", "author_handle", "content", "title", "url",
                    "created_at", "post_type", "media_urls", "hashtags", "mentions", "engagement",
                    "value_score", "quality_score", "sentiment", "topic", "content_type",
                    "is_saved", "saved_at", "analysis_timestamp", "ai_service_used",
                    "key_concepts", "tags", "sentiment_analysis", "content_summary",
                    "insights", "action_items", "recommendations", "educational_value",
                    "is_rewrite_candidate", "rewrite_status", "rewritten_content", "rewrite_notes",
                    "updated_timestamp"
                ]
                ...
                if self.supabase:
                    try:
                        ...
                        for app_field, supabase_field in field_mapping.items():
                            if app_field in post_data and post_data[app_field] is not None:
                        ...
                        # Handle array fields
                        array_fields = ['media_urls', 'hashtags', 'mentions', 'target_social_media', 'topics', 'related_skills', 'suggested_tags']
                        ...
                        if 'engagement' in post_data and post_data['engagement']:
                            supabase_data['engagement'] = json.dumps(post_data['engagement'])
                        
                        # Handle text fields that might be in the application but not in schema
                        text_fields = [
                            'content_summary', 'action_items', 'insights', 'recommendations', 
                            'educational_value', 'intelligence_analysis', 'actionable_insights',
                            'sentiment_analysis', 'analysis_timestamp', 'ai_service_used',
                            'rewrite_status', 'rewritten_content', 'rewrite_notes',
                            'created_timestamp', 'updated_timestamp', 'analysis_version',
                            'why_valuable', 'complexity_level', 'time_to_consume',
                            'actionable_items', 'learning_value', 'practical_applications',
                            'follow_up_research', 'quality_indicators', 'confidence_score',
                            'ai_service', 'intelligent_value_score', 'learning_recommendations'
                        ]
