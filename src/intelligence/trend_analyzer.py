...
class TrendAnalyzer:
    ...
    def analyze_trends(
        self, posts: List[Dict[str, Any]], time_window_hours: int = 24
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(
            f"   Found {len(recent_posts)} recent posts (last {time_window_hours}h)"
        )
    ...
    def _match_opinion(self, keyword: str) -> Dict:
        ...
        if isinstance(current, str):
            return {"angle": f"Your perspective on {keyword}", "opinion": current}
...
def demo_trend_analysis():
    ...
    logger.info("\n" + "=" * 100)
    logger.debug("🔍 TREND ANALYZER DEMO")
    logger.info("=" * 100)
    ...
    logger.info(f"\n{'='*100}")
    logger.info("💡 CONTENT SUGGESTIONS")
    logger.info("=" * 100)
    ...
    if not analysis["content_suggestions"]:
        ...
        logger.info(
            "   Suggestions appear when trending topics match your opinions in config/opinions.json"
        )
    else:
        for idx, suggestion in enumerate(analysis["content_suggestions"], 1):
            logger.info(f"\n{idx}. TREND: {suggestion['trend'].upper()}")
            logger.info(f"   Observation: {suggestion['observation']}")
            logger.info(f"   Your angle: {suggestion['your_angle']}")
            logger.info(f"\n   Your opinion:")
            logger.info(f"   {suggestion['your_opinion'][:150]}...")
            logger.info(f"\n   Suggested draft:")
            logger.info(f"   {suggestion['suggested_content']}")
            logger.info(f"\n   Examples of what people are saying:")
            for ex in suggestion["examples"]:
                logger.info(f"      • [{ex['platform']}] {ex['snippet'][:80]}...")

    logger.info(f"\n{'='*100}")
    logger.info(f"✅ Analysis complete")
    logger.info(
        f"   Filtered out {analysis['avoided_sensitive_topics']} sensitive topics (political/religious)"
    )
    logger.info("=" * 100)
