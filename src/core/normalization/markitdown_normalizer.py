"""
MarkItDown-based content normalizer for BEYONDLINES.
    ...
    def __init__(self):
        ...
        try:
            ...
            if sys.version_info < (3, 10):
                logger.warning(
                    f"MarkItDown requires Python 3.10+, current version is {sys.version_info}"
                )
                logger.warning(
                    "Content normalization will be limited without MarkItDown"
                )
                return
            ...
            logger.warning(
                "To install MarkItDown, you need Python 3.10+ and run: pip install 'markitdown[all]'"
            )
    ...
    def normalize_content(self, content: str, source_url: Optional[str] = None) -> str:
        ...
        try:
            ...
            result = self.markitdown.convert_stream(
                io.BytesIO(content_bytes), url=source_url, file_extension=".html"
            )
        ...
        for field in content_fields:
            if field in normalized_item and normalized_item[field]:
                try:
                    ...
                    normalized_content = self.normalize_content(
