...
class AsyncResearchEngine:
    ...
    async def research_async(self, query: str) -> ResearchResponse:
        ...
        try:
            ...
            result: PerplexityResult = await loop.run_in_executor(
                self.executor, 
                lambda: asyncio.run(agent.research(query))
            )
    ...
    async def research_multiple_async(self, queries: List[str]) -> List[ResearchResponse]:
        """
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                processed_results.append(ResearchResponse(
                    query=queries[i],
                    answer="",
                    sources=[],
                    follow_up_questions=[],
    ...
    def format_response(self, response: ResearchResponse) -> str:
        ...
        ]) if response.sources else "  No sources found."
        
        # Format follow-up questions
        formatted_questions = "\n".join([
            f"  {i+1}. {question}"
            for i, question in enumerate(response.follow_up_questions[:5])
        ]) if response.follow_up_questions else "  No follow-up questions generated."
...
async def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python async_research_engine.py <query>")
        print("Example: python async_research_engine.py 'Compare Qwen2.5 and Llama3'")
        return
    
    query = " ".join(sys.argv[1:])
    
    print("Initializing async research engine...")
    engine = AsyncResearchEngine()
    
    print(f"Researching: {query}")
    response = await engine.research_async(query)
    
    print(engine.format_response(response))
