...
class ThreadSummarizer:
    ...
    def __init__(self):
        ...
        logger.info("🧵 Thread Summarizer initialized")
    ...
    def summarize_thread(
        ...
        logger.info(f"🧵 Summarizing {platform} thread by {author}")

        # Try AI services in order of preference
        try:
            # Try Ollama first (local)
            if self.ai_service.ollama_url:
                result = self.ai_service.summarize_with_ollama(
            ...
            if self.ai_service.mistral_key:
                result = self.ai_service.summarize_with_mistral(
                    content, author, platform
                )
                if result.confidence > 0.5:
                    return result
            ...
            if self.ai_service.gemini_key:
                result = self.ai_service.summarize_with_gemini(
                    content, author, platform
                )
                if result.confidence > 0.5:
                    return result
            ...
            logger.error(f"⚠️ AI summarization failed: {e}")
