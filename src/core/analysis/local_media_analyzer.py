...
class LocalMediaAnalyzer:
    ...
    def analyze_post_media(self, post: Dict) -> Dict:
        ...
        try:
            ...
            logger.info(f"Analyzing {len(media_urls)} media items for post {post.get('id', 'unknown')}")
            ...
            for i, media_url in enumerate(media_urls):
                ...
                if analysis:
                    ...
                    if analysis.get('text_content'):
                        ai_analysis = self.ai_analyzer.analyze_text_with_ai(analysis['text_content'])
            ...
            if 'value_score' in enhanced_post:
                enhanced_post['value_score'] = min(1.0, enhanced_post['value_score'] + (total_value_boost / 100.0))
...
def main():
    ...
    print("Testing media analyzer...")
    print(f"Available: {analyzer.is_available()}")
    print(f"Capabilities: {analyzer.get_capabilities()}")
    
    if analyzer.is_available():
        result = analyzer.analyze_post_media(test_post)
        print(f"Enhanced post: {result}")
    else:
        print("Media analysis not available - missing dependencies")
