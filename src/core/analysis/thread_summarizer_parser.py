...
class ThreadSummarizerParser:
    ...
    def parse_structured_response(self, response: str, content: str) -> ThreadSummary:
        ...
        try:
            ...
            main_topic = self._extract_section(response, r'main.topic[:\s]*([^\n]+)', 'Content Analysis')
            key_points = self._extract_list_section(response, r'key.points?[:\s]*\n?(.*?)(?=\n\n|\n[A-Z]|$)', [])
            insights = self._extract_list_section(response, r'insights?[:\s]*\n?(.*?)(?=\n\n|\n[A-Z]|$)', [])
            sentiment = self._extract_section(response, r'sentiment[:\s]*([^\n]+)', 'neutral')
            action_items = self._extract_list_section(response, r'action.items?[:\s]*\n?(.*?)(?=\n\n|\n[A-Z]|$)', [])
            summary = self._extract_section(response, r'summary[:\s]*([^\n]+)', content[:100] + '...' if len(content) > 100 else content)
    ...
    def parse_ai_response(self, ai_response: str, original_content: str) -> ThreadSummary:
    ...
    def basic_summarize(self, content: str, author: str, platform: str) -> ThreadSummary:
    ...
    def _extract_list_section(self, text: str, pattern: str, default: List[str]) -> List[str]:
    ...
    def _has_structured_format(self, text: str) -> bool:
        ...
        structured_indicators = ['main topic', 'key points', 'insights', 'sentiment', 'action items']
        text_lower = text.lower()
        return sum(1 for indicator in structured_indicators if indicator in text_lower) >= 3
    ...
    def _extract_summary_from_response(self, response: str, original_content: str) -> str:
        ...
        return original_content[:100] + '...' if len(original_content) > 100 else original_content
