...
class SocialContentAnalyzer:
    def __init__(self, api_key: str = None):
        if api_key:
            ...
            if api_key:
                ...
                logger.warning(
                    "⚠️ No Gemini API key provided. Analyzer will use fallback analysis."
                )
    ...
    def analyze_social_post(self, post: SocialPost) -> Dict:
        ...
        for attempt in range(max_retries):
            try:
                logger.warning(
                    f"🤖 Analyzing post with Gemini (attempt {attempt + 1}/{max_retries})..."
                )
                ...
                if "429" in error_str and attempt < max_retries - 1:
                    logger.warning(
                        f"⏳ Rate limit hit, waiting {retry_delay} seconds before retry {attempt + 2}/{max_retries}"
                    )
                    time.sleep(retry_delay)
                    retry_delay *= 1.5  # Gentler backoff
                    continue
                    ...
                    logger.error(f"❌ Error analyzing social post: {e}")
                    return self._get_fallback_analysis(post)
    ...
    def analyze_batch(
        self, posts: List[SocialPost], batch_size: int = 2
    ) -> List[Dict]:  # Reduced batch size
        ...
        for i in range(0, len(posts), batch_size):
            ...
            logger.info(
                f"🧠 Analyzing batch {i//batch_size + 1} ({len(batch)} posts)..."
            )

            for post in batch:
                analysis = self.analyze_social_post(post)
                analyzed_posts.append({"post": post, "analysis": analysis})
