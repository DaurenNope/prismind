#!/usr/bin/env python3
"""
Content Analyzer Core for PrisMind
...
class ContentAnalyzerCore:
    ...
    def analyze_with_mistral(self, prompt: str, sentiment_scores: Dict, service: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    return self._parse_text_analysis(content, sentiment_scores, 'mistral')
            else:
    ...
    def analyze_with_ollama(self, prompt: str, sentiment_scores: Dict, service: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    return self._parse_text_analysis(content, sentiment_scores, 'ollama')
            else:
    ...
    def analyze_with_gemini(self, prompt: str, sentiment_scores: Dict, service: Dict) -> Dict[str, Any]:
    ...
    def _parse_text_analysis(self, content: str, sentiment_scores: Dict, ai_service: str) -> Dict[str, Any]:
        ...
        concepts_match = re.search(r'key[_ ]concepts?:?\s*\[([^\]]+)\]', content, re.IGNORECASE)
        if concepts_match:
            key_concepts = [c.strip(' "\'') for c in concepts_match.group(1).split(',')]
        
        # Extract insights
        insights_match = re.search(r'insights?:?\s*\[([^\]]+)\]', content, re.IGNORECASE)
        if insights_match:
            insights = [i.strip(' "\'') for i in insights_match.group(1).split(',')]
        
        # Extract action items
        actions_match = re.search(r'action[_ ]items?:?\s*\[([^\]]+)\]', content, re.IGNORECASE)
        if actions_match:
