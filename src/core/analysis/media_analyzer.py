...
class MediaAnalyzer:
    ...
    def _analyze_single_media(self, media_url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not any(ext in media_url.lower() for ext in ['.jpg', '.jpeg', '.png', '.gif', '.mp4', '.webp']):
                return None
            ...
            return {
                'url': media_url,
                'type': 'image' if any(ext in media_url.lower() for ext in ['.jpg', '.jpeg', '.png', '.gif', '.webp']) else 'video',
                'analysis': 'Media detected but AI vision not available',
                'confidence': 0.3
            }
            
        except Exception as e:
            print(f"⚠️ Error analyzing media {media_url}: {e}")
            return None
    ...
    def _analyze_with_gemini_vision(self, media_url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not vision_model:
                return {'url': media_url, 'analysis': 'Vision model not available', 'confidence': 0.0}
            
            # Download media content
            response = requests.get(media_url, timeout=10)
            if response.status_code != 200:
                return {'url': media_url, 'analysis': 'Failed to download media', 'confidence': 0.0}
            ...
            return {'url': media_url, 'analysis': f'Analysis failed: {str(e)}', 'confidence': 0.0}
    
    def _extract_media_insights(self, media_analyses: List[Dict[str, Any]]) -> List[str]:
