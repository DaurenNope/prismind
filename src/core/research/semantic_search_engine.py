#!/usr/bin/env python3
"""
Semantic Search Engine for PrisMind
...
class SemanticSearchEngine:
    ...
    def search_similar_content(self, query: str, contents: List[Dict[str, Any]], 
                             top_k: int = 10, min_similarity: float = 0.3) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            similarities = self.encoder.calculate_similarity(query_embedding, content_embeddings)
            
            # Create results with similarity scores
            results = []
            for i, (content, similarity) in enumerate(zip(valid_contents, similarities)):
    ...
    def find_related_topics(self, query: str, contents: List[Dict[str, Any]], 
        ...
        try:
            # Get similar content
            similar_content = self.search_similar_content(query, contents, top_k=top_k*2)
    ...
    def cluster_content(self, contents: List[Dict[str, Any]], 
        ...
        try:
            ...
            logger.info(f"📊 Clustered {len(valid_contents)} items into {n_clusters} clusters")
            return valid_contents
    ...
    def get_content_summary(self, contents: List[Dict[str, Any]], 
        ...
        try:
            ...
            total_text_length = sum(len(content.get('text', '')) for content in contents)
