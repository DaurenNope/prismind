#!/usr/bin/env python3
"""
Content Insights for BEYONDLINES
...
class ContentInsights:
    ...
    def generate_quality_recommendations(
        ...
        if avg_quality < 0.5:
            recommendations.append(
                "Focus on improving content quality - current average is below 50%"
            )

        if avg_value < 0.4:
            recommendations.append(
                "Content value is low - consider curating higher-value sources"
            )

        if low_quality_count > high_quality_count:
            recommendations.append(
                "More low-quality content than high-quality - review collection sources"
            )

        if total_count < 50:
            recommendations.append(
                "Limited content sample - collect more data for better analysis"
            )

        if avg_quality > 0.7 and avg_value > 0.6:
            recommendations.append(
                "Excellent content quality - maintain current collection strategy"
            )
    ...
    def identify_improvement_areas(
        ...
        # Engagement analysis
        low_engagement = [
            c for c in contents if sum(c.get("engagement", {}).values()) < 5
        ]
    ...
    def identify_success_factors(self, contents: List[Dict[str, Any]]) -> List[str]:
        ...
        if high_quality:
            ...
            success_factors.append(
                f"High-quality content often comes from {top_platform}"
            )
        ...
        # Engagement analysis
        high_engagement = [
            c for c in contents if sum(c.get("engagement", {}).values()) > 50
        ]
        if high_engagement:
    ...
    def generate_insights_summary(
        ...
        if content_patterns:
            ...
            if platform_dist:
                ...
                summary += (
                    f"- Most content from {top_platform[0]} ({top_platform[1]} posts)\n"
                )
            ...
            if topic_dist:
                ...
                summary += (
                    f"- Most common topic: {top_topic[0]} ({top_topic[1]} posts)\n"
                )
