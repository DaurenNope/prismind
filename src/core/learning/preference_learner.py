#!/usr/bin/env python3
"""
Preference Learning for BEYONDLINES
...
class PreferenceLearner:
    ...
    def _analyze_content_types(self, posts: List) -> Dict[str, int]:
        ...
        for post in posts:
            ...
            if any(
                word in content for word in ["tutorial", "guide", "how to", "learn"]
            ):
                types["tutorials"] += 1
            elif any(
                word in content
                for word in ["announces", "released", "launches", "breaking"]
            ):
                types["news"] += 1
            elif any(
                word in content for word in ["research", "study", "paper", "findings"]
            ):
                types["research"] += 1
            elif any(
                word in content
                for word in ["discuss", "thoughts", "opinion", "what do you think"]
            ):
                types["discussion"] += 1
            elif any(
                word in content
                for word in ["tool", "library", "framework", "api", "cli"]
            ):
                types["tools"] += 1
            else:
                types["other"] += 1
    ...
    def should_include_post(
        self, post_content: str, preferences: Dict[str, Any]
    ) -> bool:
        """
