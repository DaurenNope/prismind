...
class TwitterExtractorPlaywright(SocialExtractorBase):
    ...
    async def get_saved_posts(self, limit: int = 50, skip_cached_ids: set = None) -> List[SocialPost]:
        ...
        posts = []
        processed_tweet_ids = set(skip_cached_ids) if skip_cached_ids else set()  # Create a copy to avoid modifying the input set
    ...
    async def _extract_tweet_data(self, tweet_element, is_saved: bool = True) -> Optional[SocialPost]:
        ...
        try:
            ...
            if time_element:
                ...
                if not tweet_id:
                    try:
                        ...
                        article_element = await tweet_element.query_selector('xpath=ancestor-or-self::article')
                        if article_element:
                            tweet_id = await article_element.get_attribute('data-tweet-id')
            ...
            post_id = f"twitter_{tweet_id}" if tweet_id else f"twitter_{int(datetime.now().timestamp())}"
            ...
            return SocialPost(
                ...
                url=tweet_url or f"https://x.com/i/web/status/{tweet_id}" if tweet_id else "",
                post_type='tweet',
                ...
                analysis={
                    'tweet_id': tweet_id,
                    'tweet_url': tweet_url or f"https://x.com/i/web/status/{tweet_id}" if tweet_id else ""
                }
            )
