...
"""
Discovery Engine for BEYONDLINES
...
class DiscoveryEngine:
                ...
                relevant_posts.append(
                    {
                        ...
                        "relevance_score": topic_matches[0]["score"]
                        if topic_matches
                        else 0,
                    }
                )
    ...
    def discover_from_posts(
        self, posts: List[SocialPost], value_scores: Optional[Dict[str, float]] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        if not self.should_discover():
            logger.info("⏸️ Discovery paused (daily limit or too soon)")
            return []

        logger.debug(f"🔍 Discovery: Evaluating {len(posts)} posts...")

        # Step 1: Filter by relevance
        relevant = self.filter_by_relevance(posts)
        logger.info(f"   📊 {len(relevant)} relevant posts found")

        # Step 2: Filter by quality
        quality = self.filter_by_quality(relevant, value_scores)
        logger.info(f"   ⭐ {len(quality)} high-quality posts")
        ...
        logger.info(f"   ✅ {len(quality)} posts added to intelligence feed")
        logger.info(f"   📈 Total discovered today: {self.discovered_today}/{max_daily}")
    ...
    def reset_daily_counter(self):
        ...
        self.discovered_today = 0
        logger.info("🔄 Daily discovery counter reset")
