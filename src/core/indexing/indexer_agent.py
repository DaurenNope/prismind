"""
Indexer Agent for BEYONDLINES - handles content indexing and vector storage.
"""
...
class IndexerAgent:
    ...
    def index_content(self, post) -> bool:
        ...
        try:
            ...
            content_for_embedding = (
                self.embedding_service.prepare_content_for_embedding(post_data)
            )

            # Generate content hash
            content_hash = self.embedding_service.get_content_hash(
                content_for_embedding
            )
            ...
            if success:
                ...
                logger.error(
                    f"Failed to store embedding in database: {content_hash[:8]}..."
                )
    ...
    def index_content_batch(self, posts: List) -> Dict[str, int]:
        ...
        """
        stats = {"total": len(posts), "successful": 0, "failed": 0}
