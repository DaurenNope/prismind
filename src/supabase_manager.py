...
class SupabaseManager:
        ...
        try:
            ...
            if url:
                ...
                if url_response.data:
                    print(f"🔍 URL duplicate found: {url}")
                    return True
            ...
            if author.upper() == "ZERO IKA" and len(response.data) > 0:
                print(f"🔍 ZERO IKA duplicate check: {content[:50]}...")
                for existing_post in response.data:
                    print(f"    Existing post ID: {existing_post.get('id')}")
                    print(f"    Created at: {existing_post.get('created_at')}")
                    print(f"    URL: {existing_post.get('url')}")
            ...
            if len(response.data) > 0:
                ...
                print(f"🔍 Duplicate found: {author} - {content[:50]}...")
                print(f"    Existing in categories: {existing_categories}")
                if category:
                    print(f"    Checking within category: '{category}'")
            else:
                print(f"✅ New content: {author} - {content[:50]}...")
    ...
    def insert_post(self, post_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if not post_data:
                print(f"❌ Supabase insert_post: Empty post_data provided")
                return {}

            # Ensure we have required fields
            if not post_data.get("title") and not post_data.get("content"):
                print(f"❌ Supabase insert_post: Missing both title and content")
                print(f"   Post ID: {post_data.get('post_id', 'unknown')}")
                print(f"   Keys present: {list(post_data.keys())}")
                return {}

            # Debug: Check if embedding exists in incoming data
            has_emb_input = 'embedding' in post_data and post_data['embedding'] is not None
            if has_emb_input:
                print(f"🔍 insert_post: Received embedding ({len(post_data['embedding'])} dims)")
            else:
                print(f"⚠️  insert_post: NO embedding in post_data")
            ...
            if url and self.check_duplicate_by_url(url):
                if is_enriched_update:
                    print(f"   🔄 Duplicate URL found, will UPDATE with enriched data: {url}")
            ...
            # Then check by content/author
                ...
                if is_enriched_update:
                    print(f"   🔄 Duplicate found, will UPDATE with enriched data")
                    # Don't return, proceed to update
                else:
                    print(f"   ⏭️  Duplicate detected: {author} - {content[:50]}...")
                    return {}  # Return empty dict to indicate duplicate
            else:
                print(
            ...
            # Filter out fields that analyzer produces but Supabase schema doesn't have
            fields_to_exclude = {
                'id', 'action_items', 'actionable_items', 'learning_value', 'practical_applications',
                ...
                'username', 'quality_score', 'engagement'  # Use author_handle; quality_score->content_quality_score; engagement as dict not supported
            }
            
            # Clean the data - remove None values and ensure proper types
            clean_data = {}
            for key, value in post_data.items():
                if value is not None and key not in fields_to_exclude:
            ...
            print(f"📤 Supabase: {action} with {len(clean_data)} fields{emb_preview}")
            ...
            try:
                ...
                if "schema cache" in error_str or "column" in error_str.lower():
                    print(f"❌ Supabase insert_post ERROR: Schema mismatch - {api_error}")
                    print(f"   Post ID: {post_data.get('post_id', 'unknown')}")
                    print(f"   Platform: {post_data.get('platform', 'unknown')}")
                    print(f"   URL: {post_data.get('url', 'unknown')}")
                    print(f"   Post data keys: {list(post_data.keys())}")
                    print(f"   This indicates the Supabase table schema needs to be updated")
                    print(f"   Run the schema update script at supabase_schema_update.sql")
                    return {}
                else:
                    raise api_error
            ...
            print(f"❌ Supabase insert_post ERROR: {e}")
            print(f"   Post ID: {post_data.get('post_id', 'unknown')}")
            print(f"   Platform: {post_data.get('platform', 'unknown')}")
