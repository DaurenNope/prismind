...
class PromptBuilder:
    ...
    def _render_custom_prompt(self, template: str, request: RewriteRequest, plan: RewritePlan) -> str:
        ...
        # Extract and normalize language from analyzed_content first
        analyzed_lang = request.analyzed_content.get("language", "").lower()
        if analyzed_lang in ["ru", "russian", "ru-ru"]:
            language_for_prompt = "russian"
        elif analyzed_lang in ["en", "english", "en-us", "en-us"]:
            language_for_prompt = "english"
        else:
            # Fallback to persona language, then default to English (not Russian)
            language_for_prompt = request.persona.language or "english"
        
        # Extract analysis context
        key_concepts = request.analyzed_content.get("key_concepts", [])
        action_items = request.analyzed_content.get("action_items", [])
        topics = request.analyzed_content.get("topics", [])
        
        # Simple placeholder replacement
        replacements = {
            "{persona_name}": request.persona.name or request.persona.key,
            "{persona_language}": language_for_prompt,
            ...
            "{key_concepts}": ', '.join(key_concepts[:5]) if key_concepts else '',
            "{action_items}": ', '.join(action_items[:3]) if action_items else '',
