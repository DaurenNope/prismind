...
class RewriteAnalytics:
    ...
    def log_rewrite(self, rewrite_data: Dict[str, Any]):
        ...
        try:
            ...
            logger.debug(
                f"📊 Logged rewrite: {rewrite_data['persona']} / {rewrite_data.get('content_type')}"
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if not self.rewrite_log.exists():
            return {"total_rewrites": 0, "message": "No rewrites logged yet"}
        ...
        all_quality_scores = [
            r["quality_score"] for r in rewrites if "quality_score" in r
        ]
        avg_quality = (
        ...
        example_performance = defaultdict(
            lambda: {"uses": 0, "avg_quality": 0, "qualities": []}
        )
        for r in rewrites:
        ...
        return {
            ...
            "by_persona": {
                persona: {
                    "count": data["count"],
                    "avg_quality": round(
                        sum(data["quality_scores"]) / len(data["quality_scores"]), 2
                    )
    ...
    def save_stats(self):
        ...
        try:
            ...
            logger.info(
                f"📊 Saved analytics stats: {stats.get('total_rewrites')} rewrites"
            )
        except Exception as e:
