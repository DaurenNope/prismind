...
async def post_to_twitter_playwright(content: str) -> Dict:
    ...
    try:
        ...
        cookie_file = (
            ...
            or f"config/cookies/twitter_cookies_{username}.json"
            if username
            else None
        )
        ...
        # Use headless mode in Docker/CI, or if HEADLESS_MODE env var is set
        ...
        if cookie_file and Path(cookie_file).exists():
            with open(cookie_file, "r") as f:
                ...
                if isinstance(cookies_data, dict):
                    ...
                    cookies = cookies_data.get(
                        "cookies", cookies_data.get("data", [cookies_data])
                    )
        ...
        await page.goto(
            "https://twitter.com/compose/tweet", wait_until="domcontentloaded"
        )
        ...
        try:
            ...
            if password:
                await page.goto(
                    "https://twitter.com/i/flow/login", wait_until="domcontentloaded"
                )
                await page.wait_for_timeout(2000)
                ...
                try:
                    ...
                    return {
                        "success": False,
                        "error": f"Login failed (username): {str(e)}",
                    }
                ...
                try:
                    ...
                    try:
                        ...
                        if (
                            "could not log" in page_text.lower()
                            or "try again later" in page_text.lower()
                        ):
                            # Take screenshot for debugging
                            await page.screenshot(
                                path="logs/twitter_post_auth_error.png"
                            )
                            return {
                                "success": False,
                    ...
                    error_msg = str(e)
                    if (
                        "could not log" in error_msg.lower()
                    ...
                    return {
                        "success": False,
                        "error": f"Login failed (password): {error_msg}",
                    }
                ...
                try:
                    ...
                    if cookie_file:
                        try:
                            ...
                            if cookies:
                                Path(cookie_file).parent.mkdir(
                                    parents=True, exist_ok=True
                                )
                                with open(cookie_file, "w") as f:
                                    json.dump(cookies, f, indent=2)
                                logger.info(
                                    f"✅ Saved {len(cookies)} cookies after password login to {cookie_file}"
                                )
                        except Exception as e:
                            logger.warning(
                                f"⚠️ Failed to save cookies after login: {e}",
                                exc_info=True,
                            )
                except Exception as e:
                    logger.error(
                        f"❌ Could not find compose textarea after login: {e}",
                        exc_info=True,
                    )
                    return {
                        "success": False,
                        "error": f"Could not find compose textarea after login: {e}",
                    }
            else:
                return {
                    "success": False,
                    "error": "Not logged in and no password provided",
                }
        ...
        if "/status/" in current_url:
            ...
            username_from_page = current_url.split("twitter.com/")[1].split("/status/")[
                0
            ]
            ...
            try:
                ...
                if status_elements:
                    ...
                    link = await status_elements[0].query_selector(
                        'a[href*="/status/"]'
                    )
                    if link:
            ...
            try:
                ...
                if cookies:
                    ...
                    with open(cookie_file, "w") as f:
                        ...
                        json.dump(cookies, f, indent=2)
                    logger.info(
                        f"✅ Refreshed {len(cookies)} cookies saved to {cookie_file}"
                    )
            except Exception as e:
                logger.error(
                    f"⚠️ Failed to save cookies: {e}"
                )  # Don't fail on cookie save error
        ...
        try:
            ...
            logger.warning(
                f"⚠️ Failed to save cookies on error: {cookie_error}", exc_info=True
            )
