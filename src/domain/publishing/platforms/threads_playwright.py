...
async def post_to_threads_playwright(
    ...
    try:
        ...
        headless_mode = os.getenv("THREADS_HEADLESS", "true").lower() in (
            "true",
            "1",
            "yes",
        )
        ...
        if not headless_mode:
            logger.debug("🔍 DEBUG MODE: Browser will be visible (headless=False)")
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Error: {e}")
                # Fallback to threads.net if threads.com fails
                await page.goto(
            ...
            if (
                ...
                if password:
                    logger.info(
                        "Cookies didn't work, trying username/password login..."
                    )
                    await page.goto(
                    ...
                    if not username_input:
                        return {
                            "success": False,
                            "error": "Login failed: Could not find username input",
                        }
                    ...
                    if not password_input:
                        return {
                            "success": False,
                            "error": "Login failed: Could not find password input",
                        }
                    ...
                    if not login_button:
                        return {
                            "success": False,
                            "error": "Login failed: Could not find login button",
                        }

                    await login_button.click()
                    await asyncio.sleep(3)

                    # Navigate to home after login (try threads.com first)
                    try:
                        await page.goto(
                            "https://www.threads.com/",
                            wait_until="domcontentloaded",
                    ...
                    if cookie_file and context:
                        try:
                            ...
                            logger.info(
                                f"✅ Saved {len(cookies)} cookies after password login"
                            )
                        except Exception as e:
                    ...
                    return {
                        "success": False,
                        "error": "Not logged in - cookies expired, need password",
                    }
        ...
        # Check if we're actually logged in - if redirected to login, fail early
        if "login" in current_url.lower() or "accounts/login" in current_url.lower():
            logger.error("❌ Not logged in - redirected to login page")
            return {
                "success": False,
                "error": "Not logged in - cookies expired or invalid. Please refresh cookies.",
            }

        # Wait for page to actually have content (not just empty DOM)
        ...
        try:
            ...
            if not body_text or len(body_text.strip()) < 10:
                logger.warning("⚠️ Page appears empty - may not be loaded")
        ...
        try:
            ...
            if len(all_buttons) == 0:
                logger.error(
                    "❌ Page has no clickable elements - likely not logged in or page not loaded"
                )
                # Take screenshot for debugging
                try:
                    await page.screenshot(
                        path="logs/threads_empty_page.png", full_page=True
                    )
                ...
                try:
                    ...
                    if text or aria_label:
                        ...
                        if text and (
                            ...
                            or "post" in text.lower()
                            or "thread" in text.lower()
                        ):
                            compose_candidates.append((i, text, aria_label))
                        elif aria_label and (
                            "compose" in str(aria_label).lower()
                            or "new" in str(aria_label).lower()
                            or "post" in str(aria_label).lower()
                            or "thread" in str(aria_label).lower()
                        ):
                            compose_candidates.append((i, text, aria_label))
                except Exception as e:
            ...
            if compose_candidates:
                logger.info(
                    f"🎯 Found {len(compose_candidates)} potential compose buttons:"
                )
                for idx, candidate in enumerate(compose_candidates):
                    i, text, aria = candidate
                    logger.info(
                        f"  Candidate {idx}: text='{text[:40] if text else ''}' aria='{aria or ''}'"
                    )
        except Exception as e:
            logger.warning(f"⚠️  Could not inspect buttons: {e}")
        ...
        for selector in compose_selectors:
            try:
                ...
                # Use shorter timeout to avoid long waits - 2 seconds per selector max
                ...
                # Don't log every failed selector - too verbose
        ...
        if not compose_button:
            try:
                logger.debug(
                    "🔍 Trying to find compose button by aria-label 'Empty text field'..."
                )
                compose_button = await page.query_selector(
                    '[aria-label*="Empty text field"]'
                )
                if compose_button:
                    is_visible = await compose_button.is_visible()
        ...
        if compose_button:
            ...
            try:
                ...
                logger.info("✅ Clicked with force=True")
            except Exception as e1:
                logger.error(f"⚠️ Force click failed: {e1}")
                try:
                    ...
                    logger.error(f"⚠️ JavaScript click failed: {e2}")
                    try:
                        ...
                        logger.info("✅ Clicked after scrolling")
                    except Exception as e3:
                        logger.error(f"⚠️ Normal click failed: {e3}")
                        # Last resort: click parent element
                        try:
                            parent = await compose_button.evaluate_handle(
        ...
        try:
            ...
            textarea = await page.wait_for_selector(
                '[contenteditable="true"]', timeout=15000, state="visible"
            )
            logger.info("✅ Compose modal opened, textarea found!")
            await page.wait_for_timeout(1000)  # Wait for it to be ready
        except Exception as e:
            logger.warning(
                f"Compose modal textarea didn't appear after button click: {e}"
            )
            textarea = None

        # If compose button wasn't found or textarea didn't appear, try direct navigation
        if not compose_button or not textarea:
            logger.info(
                "No compose button found or textarea didn't appear - trying direct navigation to /compose..."
            )
            try:
                # Try navigating directly to compose page
                current_url = page.url
                base_url = (
                    current_url.split("/")[0] + "//" + current_url.split("/")[2]
                )  # Get domain
                compose_url = f"{base_url}/compose"
                logger.info(f"Navigating directly to: {compose_url}")
                await page.goto(
                    compose_url, wait_until="domcontentloaded", timeout=20000
                )
                await page.wait_for_timeout(2000)

                # Try to find textarea directly on compose page
                try:
                    textarea = await page.wait_for_selector(
                        '[contenteditable="true"]', timeout=5000, state="visible"
                    )
                    if textarea:
                        logger.info("✅ Found textarea on compose page!")
                except Exception as e:
                    logger.debug(f"Textarea not found on compose page: {e}")
            except Exception as nav_error:
                logger.debug(f"Direct navigation to compose failed: {nav_error}")

        # Final check - try to find textarea directly (we may be on /compose page already)
        if not textarea:
            logger.info("Trying to find textarea directly on current page...")
            try:
                textarea = await page.wait_for_selector(
                    '[contenteditable="true"]', timeout=5000, state="visible"
                )
        ...
        if not textarea:
            ...
            try:
                await page.screenshot(
                    path="logs/threads_no_compose_button.png", full_page=True
                )
        ...
        try:
            ...
            try:
                ...
                logger.info("✅ Triggered input events")
            ...
            if len(actual_content) > 0:
                ...
                await textarea.evaluate(
                    f"el => {{ el.textContent = {json.dumps(content)}; }}"
                )
                await page.wait_for_timeout(500)
                actual_content = await textarea.inner_text()
                logger.info(f"✅ Content typed (fallback): {len(actual_content)} chars")
                logger.info(f"✅ Content typed (fallback): {actual_content[:50]}...")

            # Wait longer for button to become ready after content is typed
            await page.wait_for_timeout(2000)
            logger.info("⏳ Waiting for post button to be ready after typing...")
        except Exception as e:
        ...
        for btn_info in all_post_buttons:
            ...
            try:
                ...
                parent = await btn.evaluate_handle(
                    'el => el.closest(\'div[role="dialog"], div[class*="modal"], div[class*="compose"]\')'
                )
            ...
            for btn_info in all_post_buttons:
                ...
                try:
                    ...
                    if is_enabled and is_visible and not is_disabled:
                        ...
                        if (
                            "post" in text_lower
                            and "share" not in text_lower
                            and "reply" not in text_lower
                        ):
                            post_button = btn
                            logger.info(
        ...
        try:
            ...
            logger.debug(
                f"🔍 Post button debug: tag={button_tag}, text='{button_text}', aria='{button_aria}'"
            )

            # Check if button has click handler
            has_onclick = await post_button.evaluate(
                "el => el.onclick !== null || el.hasAttribute('onclick')"
            )
            has_listener = await post_button.evaluate(
                "el => el.getAttribute('data-reactid') !== null || el.getAttribute('data-testid') !== null"
            )
            logger.debug(
                f"🔍 Post button has onclick: {has_onclick}, has listener: {has_listener}"
            )
        except Exception as debug_error:
            logger.error(f"⚠️ Button debug failed: {debug_error}")
        ...
        try:
            ...
            logger.info("🖱️ Clicking post button with JavaScript...")
            ...
            logger.info("✅ Post button clicked with JavaScript (with events)")
        except Exception as e1:
            logger.error(f"⚠️ JavaScript click failed: {e1}")
            # Only try other methods if first attempt truly failed
            try:
                # Force click as fallback
                await post_button.click(force=True)
                logger.info("✅ Post button clicked with force=True")
            except Exception as e2:
                logger.error(f"⚠️ Force click failed: {e2}")
        ...
        for attempt in range(10):  # Check up to 10 times (10 seconds)
            try:
                ...
                if "/post/" in current_url:
                    ...
                    username_from_page = (
                        current_url.split("threads.net/@")[1].split("/post/")[0]
                        if "@" in current_url
                        else None
                    )
                    ...
                    post_success = True
                    logger.info(f"✅ Post URL detected: {post_url}")
                ...
                compose_modal = await page.query_selector(
                    'div[role="dialog"], [data-testid*="compose"], [aria-label*="compose"]'
                )
                textarea_still_visible = await page.query_selector(
                ...
                logger.debug(
                    f"🔍 Modal check - modal_open: {bool(modal_open)}, compose_modal: {bool(compose_modal)}, textarea_visible: {bool(textarea_still_visible)}"
                )

                if not modal_still_open:
                    # Modal closed - might be success!
                    logger.info("✅ Modal closed - checking if post appeared...")
                ...
                for message_selector in message_selectors:
                    try:
                        ...
                        if message_element:
                            ...
                            if (
                                message_text and len(message_text) < 300
                            ):  # Reasonable message length
                                logger.info(f"📄 Message detected: {message_text}")
                                ...
                                if "posting" in message_text.lower():
                                    ...
                                    current_url = page.url
                                    if "/post/" in current_url:
                                        post_id = current_url.split("/post/")[-1].split(
                                            "?"
                                        )[0]
                                        username_from_page = (
                                            current_url.split("threads.net/@")[1].split(
                                                "/post/"
                                            )[0]
                                            if "@" in current_url
                                            else None
                                    ...
                                    logger.info(
                                        f"✅ Post successful - 'Posting...' detected, assuming success"
                                    )
                                    break
                                ...
                                is_button_text = (
                                    "post" in message_text.lower()
                                    and len(message_text.strip()) <= 5
                                )
                                is_success_message = (
                                    any(
                            ...
                            if is_success_message:
                                ...
                                textarea_check = await page.query_selector(
                                    '[contenteditable="true"]'
                                )
                                if not textarea_check:
                                    ...
                                    logger.warning(
                                        "⚠️ Success message but modal still open - post may have been created"
                                    )
                                    ...
                                    try:
                                        ...
                                        if content[:50].strip() in page_content:
                                            post_success = True
                                            logger.info("✅ Post confirmed in feed!")
                                            break
                                    except Exception as e:
                            ...
                            # Check if it's an error message
                                ...
                                logger.error(f"❌ Error detected: {message_text}")
                ...
                if not textarea_still_visible and not post_success:
                    logger.warning(
                        "⚠️ Textarea disappeared but no success message - checking feed..."
                    )
                    await page.wait_for_timeout(2000)
                    ...
                    if modal_open or compose_modal:
                        logger.warning(
                            "⚠️ Modal still open but textarea gone - might be confirmation screen"
                        )
                        # Wait a bit more and check again
                        await page.wait_for_timeout(3000)

                await page.wait_for_timeout(1000)  # Wait 1 second before next check
                logger.warning(f"⏳ Waiting for post confirmation... ({attempt + 1}/10)")
        ...
        if "/post/" in current_url:
            ...
            if username_from_page:
                post_url = (
                    f"https://www.threads.net/@{username_from_page}/post/{post_id}"
                )
            else:
                post_url = current_url
                post_success = True
                logger.info(f"✅ Post URL found after extended wait: {post_url}")

        if not post_success:
            # Take screenshot to debug
            try:
                await page.screenshot(
                    path="logs/threads_post_failed.png", full_page=True
                )
        ...
        if cookie_file and context:
            try:
                ...
                with open(cookie_file, "w") as f:
                    json.dump(cookie_data, f, indent=2)
                logger.info(
                    f"✅ Saved {len(cookies)} cookies to {cookie_file} (mimesis format)"
                )
            except Exception as e:
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(
                    asyncio.run,
                    post_to_threads_playwright(content, image_url=image_url),
                )
