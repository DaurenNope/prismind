...
try:
    from .ml_insights import ContentAnalyzer, ContentInsight, EngagementPrediction
except ImportError:
    # Handle direct script execution
    from ml_insights import ContentAnalyzer, ContentInsight, EngagementPrediction
    ...
    async def generate_content(self, persona_data: Dict[str, Any], content: Dict[str, Any], platform: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if topic:
                ...
                # To re-enable: Resolve dependency conflicts between sentence_transformers and huggingface_hub.
                # Workaround: Using persona examples as fallback for semantic similarity.
...
async def test_dynamic_rewriter():
    ...
    logger.info(f"✅ Persona created: {persona_data['name']}")
    logger.info(f"Quality metrics: {persona_data['quality_metrics']}")
    ...
    logger.info(f"Content: {result.get('content', 'No content generated')}")
