...
class LibrarianBookAgent:
    ...
    async def discover_and_analyze(
        self, book_title: str, author: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    async def _discover_and_analyze_internal(
        self, book_title: str, author: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        Discover, download, and analyze a book
        ...
        try:
            ...
            report = {
                ...
                # Rewrite angles
                "rewrite_angles": self._generate_book_rewrite_angles(
                    book_title, author, analysis
                ),
            }
            ...
            return {"title": book_title, "status": "error", "error": str(e)}
    ...
    async def _search_annas_archive(
        self, title: str, author: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    async def _search_libgen(
        ...
        try:
            ...
            params = {"req": title, "res": 25, "view": "simple"}
    ...
    async def _extract_text(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"full_text": "", "word_count": 0, "page_count": 0, "chapters": []}

    async def _analyze_book(
        self, text_content: Dict, title: str, author: Optional[str]
    ) -> Dict[str, Any]:
...
Provide structured analysis:"""
    ...
    def _generate_insights(self, analysis: Dict, text_content: Dict) -> List[str]:
        ...
        if key_concepts:
            insights.append(
                f"Study {len(key_concepts)} core concepts for deep understanding"
            )
    ...
    def _generate_book_tldr(
        self, title: str, author: Optional[str], analysis: Dict
    ) -> str:
    ...
    def _generate_applications(self, analysis: Dict) -> List[str]:
        ...
        if key_concepts:
            applications.append(
                f"Apply {len(key_concepts)} core concepts to daily work"
            )
    ...
    def _identify_book_audience(self, analysis: Dict) -> List[str]:
        ...
        if category == "self-improvement":
            audiences.extend(
                ["professionals seeking growth", "productivity enthusiasts"]
            )
    ...
    def _generate_book_rewrite_angles(
        self, title: str, author: Optional[str], analysis: Dict
    ) -> List[Dict[str, str]]:
    ...
    def get_library_stats(self) -> Dict[str, Any]:
        ...
        books = list(self.library_path.glob("*.pdf")) + list(
            self.library_path.glob("*.epub")
        )
...
async def test_librarian():
    ...
    logger.info("🧪 Testing Librarian Book Agent\n")
    ...
    logger.info(f"Processing: {book_title} by {author}\n")

    report = await agent.discover_and_analyze(book_title, author)

    logger.info("=" * 70)
    logger.info("📚 BOOK ANALYSIS REPORT")
    logger.info("=" * 70)
    logger.info("")
    ...
    if report["status"] == "analyzed":
        ...
        logger.info()
        logger.info(f"Key Concepts ({len(report['key_concepts'])}):")
        ...
        logger.info()
        logger.info(f"Category: {report['category']}")
        logger.info(f"Priority: {report['priority']}/10")
        logger.info(f"Reading time: {report['reading_time_hours']:.1f} hours")
    ...
    logger.info(f"  Books: {stats['total_books']}")
    logger.info(f"  Size: {stats['total_size_mb']:.2f} MB")
    logger.info()
    logger.info("✅ Librarian agent working!")
