...
from .book_library_operations import Book, BookEntry, BookLibraryOperations
...
class PrivateBookLibrary:
    ...
    def __init__(self, library_path: str = "private_library"):
        ...
        logger.info("📚 Private Book Library initialized")

    # Basic Operations
    async def add_book_from_research(
        self, book: Book, file_path: Optional[str] = None
    ) -> BookEntry:
    ...
    async def extract_relevant_sections(
        self, book_id: str, keywords: List[str]
    ) -> List[Dict]:
        ...
        return await self.analyzer.extract_relevant_sections(
            book_id, keywords, book_entry
        )
    ...
    async def get_book_summary(self, book_id: str) -> Dict[str, Any]:
        ...
        return {"book_info": book_info, "content_analysis": content_analysis}
    ...
    async def find_books_by_topic(self, topic: str) -> List[Dict[str, Any]]:
        ...
        for book_entry in all_books:
            ...
            if (
                topic_lower in book.title.lower()
                or (book.description and topic_lower in book.description.lower())
                or any(topic_lower in cat.lower() for cat in book.categories)
                or any(topic_lower in tag.lower() for tag in book_entry.tags)
            ):
    ...
    def _calculate_relevance_score(
        self, book: Book, book_entry: BookEntry, topic: str
    ) -> float:
